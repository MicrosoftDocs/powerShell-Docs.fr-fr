...
Vous pouvez utiliser les objets exposés par l’espace de noms N:Microsoft.PowerShell.ScheduledJob pour créer une tâche planifiée, définir lorsqu’elle est exécutée et obtenir des résultats sur la tâche terminée après son exécution.

## <a name="triggering-the-job"></a>Déclenchement du travail

Spécifie la première étape de création d’une tâche planifiée lorsque la tâche doit s’exécuter. Cela en créant et en configurant un objet T:Microsoft.PowerShell.ScheduledJob.ScheduledJobTrigger. Le code suivant crée un déclencheur qui planifie un travail à exécuter une seule fois à l’avenir de 20 secondes.
...
Vous définissez un travail de Windows PowerShell en créant un dictionnaire de paramètres. Les paramètres suivants sont pris en charge.

|Nom du paramètre|Description|
|---|---|
|Name|Le nom de la tâche.|
|ScriptBock|Un bloc de script Windows PowerShell qui spécifie ce que fait le travail.|
|FilePath|Un chemin d’accès à un fichier qui contient le bloc de script Windows PowerShell qui spécifie ce que fait le travail.|
|InitializationScript|Un bloc de script Windows PowerShell qui initialise la tâche.|
...
Le code suivant crée un objet de dictionnaire de paramètre et définit les paramètres de nom et ScriptBlock.
...
## <a name="creating-the-invocation-and-job-definition-objects"></a>Création de l’appel et les objets de définition de travail

Vous créez ensuite des objets ScheduledJobInvocationInfo et ScheduledJobDefinition pour exécuter la tâche. Le code suivant illustre cela.
...
Le code suivant inscrit le travail avec le Planificateur de tâches Windows.
