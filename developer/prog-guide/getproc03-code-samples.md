...
ms.locfileid: "62081612"
---
# <a name="getproc03-code-samples"></a>Exemples de code GetProc03

Voici les exemples de code pour l’applet de commande GetProc03 exemple. Il s’agit du `Get-Process` exemple d’applet de commande décrites dans [ajoutant des paramètres de cette entrée de Pipeline de processus](../cmdlet/adding-parameters-that-process-pipeline-input.md). Cela `Get-Process` applet de commande utilise un `Name` paramètre qui accepte les entrées à partir d’un objet de pipeline, récupère les informations de processus à partir de l’ordinateur local selon les noms fournis, puis affiche des informations sur les processus à la ligne de commande.
...
|Language|Rubrique|
|--------------|-----------|
|C#|[GetProc03 (C#) exemple de Code](./getproc03-csharp-sample-code.md)|
|VB.NET|[GetProc03 exemple de Code (VB.NET)](./getproc03-vb-net-sample-code.md)|
