...
Définit comment les données dans une colonne de la ligne sont affichées.

Configuration élément (Format) élément ViewDefinitions (Format) vue (Format) élément de table (Format) élément TableRowEntries (Format) TableRowEntry, élément (Format) TableColumnItems, élément (Format) TableColumnItem élément (Format) Élément d’alignement pour TableColumnItem (Format)
...
Décalages vers la gauche les données affichées dans la colonne à gauche. (Ceci est la valeur par défaut si cet élément n’est pas spécifié).

Décalages vers la droit les données affichées dans la colonne à droite.
...
[Vue de table](./creating-a-table-view.md)
