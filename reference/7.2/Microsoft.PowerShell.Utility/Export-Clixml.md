...
La chaîne `This is a test` est envoyée dans le pipeline. `Export-Clixml` utilise le paramètre **path** pour créer un fichier XML nommé `sample.xml` dans le répertoire actif.
