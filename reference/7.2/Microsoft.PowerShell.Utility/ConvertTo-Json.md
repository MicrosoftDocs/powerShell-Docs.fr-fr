...
À partir de PowerShell 7,2, `ConvertTo-Json` émet un avertissement si la profondeur de l’objet d’entrée dépasse la profondeur spécifiée pour la commande. Cela empêche la perte de données indésirables lors de la conversion d’objets.
...
Spécifie le nombre de niveaux d'objets contenus inclus dans la représentation JSON. La valeur par défaut est 2. À partir de PowerShell 7,2, `ConvertTo-Json` émet un avertissement si le nombre de niveaux dans un objet d’entrée dépasse ce nombre.
