...
>[!NOTE]
> Dans PowerShell 3,0 et PowerShell 4,0, `Get-Help` les articles  ne peuvent pas être détectés dans les modules, sauf si le module est importé dans la session active. Il s'agit d'un problème connu. Pour obtenir des **informations sur** les articles d’un module, importez le module à l’aide de l’applet de commande `Import-Module` ou en exécutant une applet de commande incluse dans le module.
...
Le paramètre **ShowWindow** prend en charge les rubriques d’aide pour les commandes (applets de commande, fonctions, commandes CIM, scripts) et **les articles** conceptuels. Il ne gère pas l'aide du fournisseur.

Ce paramètre a été réintroduit dans PowerShell 7,0.
