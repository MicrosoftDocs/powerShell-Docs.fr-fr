...
- `Ctrl+Break`: (Arrêter tout dans ISE) s’arrête dans un script en cours d’exécution dans la console PowerShell ou Windows PowerShell ISE. Notez que la <kbd>touche Ctrl</kbd> + <kbd></kbd> dans Windows PowerShell 2,0, 3,0 et 4,0 ferme le programme. L’instruction Break All fonctionne sur les scripts locaux et distants, en cours d’exécution de manière interactive.
...
Quand vous atteignez un point d’arrêt, vous entrez l’environnement du débogueur. L’invite de commandes change de sorte qu’elle commence par « [DBG] : ».
...
- [PsBreakpoint](xref:Microsoft.PowerShell.Utility.Get-PSBreakpoint)
- [PsCallStack](xref:Microsoft.PowerShell.Utility.Get-PSCallStack)
