...
> [!NOTE]
> Cette variable est disponible uniquement lorsque la `PSAnsiRendering` fonctionnalité expérimentale IA est activée. Pour plus d’informations, consultez [about_Experimental_Features](about_Experimental_Features.md) et [utilisation des fonctionnalités expérimentales](/powershell/scripting/learn/experimental-features).

À partir de PowerShell 7,2, vous pouvez maintenant accéder `$PSStyle` à la variable automatique pour afficher et modifier le rendu de la sortie de chaîne ANSI. La variable contient les propriétés suivantes :

- **Réinitialiser** : désactive toutes les décorations
- **Arrière-plan** -objet imbriqué pour contrôler la coloration de l’arrière-plan
- **Clignotement** -active le clignotement
- **BlinkOff** : désactive le clignotement
- **Gras** -active le gras
- **BoldOff** -désactive le gras
- Objet imbriqué de **premier plan** pour contrôler la coloration de premier plan
- **Mise en forme** -contrôle la mise en forme par défaut pour les flux de sortie
- **Hidden** -s’active masqué
- **HiddenOff** -désactive le mode masqué
- **OutputRendering** -Control lorsque le rendu de sortie est utilisé
- **Inverse** : désactive l’effet inverse
- **ReverseOff** -désactive l’annulation
- **Italique** -active l’italique
- **ItalicOff** : désactive l’italique
- Souligné-active le **soulignement**
- **UnderlineOff** -désactive le soulignement

Les membres de base retournent des chaînes de séquences d’échappement ANSI mappées à leurs noms.
Les valeurs peuvent être définies de façon à autoriser la personnalisation. Par exemple, vous pouvez remplacer gras par souligné. Les noms de propriété facilitent la création de chaînes décorées à l’aide de la saisie semi-automatique par tabulation :

```powershell
"$($PSStyle.Background.LightCyan)Power$($PSStyle.Underline)$($PSStyle.Bold)Shell$($PSStyle.Reset)"
```

Les `$PSStyle.Background` `$PSStyle.Foreground` membres et sont des chaînes qui contiennent les séquences d’échappement ANSI pour les 16 couleurs de console standard, ainsi qu’une `Rgb()` méthode pour spécifier une couleur de 24 bits. Les valeurs sont définissables et peuvent contenir n’importe quel nombre de séquences d’échappement ANSI.

`$PSStyle.Formatting` est un objet imbriqué pour contrôler la mise en forme par défaut des messages de débogage, d’erreur, de commentaires et d’avertissement. Vous pouvez également contrôler des attributs tels que la mise en gras et le soulignement. Il remplace `$Host.PrivateData` le mode de gestion des couleurs pour la mise en forme du rendu. `$Host.PrivateData` continue à exister à des fins de compatibilité descendante, mais n’est pas connecté à `$PSStyle.Formatting` .

`$PSStyle.OutputRendering` est une `System.Management.Automation.OutputRendering` énumération avec les valeurs :

- **Automatique**: il s’agit de la valeur par défaut. Si l’hôte prend en charge VirtualTerminal, ANSI est toujours passé tel quel, sinon en texte brut
- **ANSI**: ANSI est toujours transmis tel quel
- **Texte brut**: les séquences d’échappement ANSI sont toujours supprimées afin qu’il ne s’agit que d’un texte brut
- **HostOnly**: il s’agit du comportement MacOS dans lequel les séquences d’échappement ANSI sont supprimées dans une sortie redirigée ou dirigée.
...
- **PSEdition** : cette propriété a la valeur « Desktop » pour PowerShell 4 et versions antérieures, ainsi que PowerShell 5,1 sur les éditions complètes de Windows. Cette propriété a la valeur « Core » pour PowerShell 6 et versions ultérieures, ainsi que PowerShell PowerShell 5,1 sur les éditions à encombrement réduit, telles que Windows nano Server ou Windows IoT.
