...
- Désactivé. désactive la configuration de session. Elle ne peut pas être utilisée pour l'accès local ou distant à l'ordinateur. Cette valeur affecte la  valeur `WSMan:\<ComputerName>\PlugIn\<SessionConfigurationName>\Enabled` **false** à la propriété Enabled de la configuration de session ().
