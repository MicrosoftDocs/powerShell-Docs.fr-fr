...
Deux paramètres de couleur ont été ajoutés pour prendre en charge la personnalisation du `ListView` dans PSReadLine 2.2.0 :

- **ListPredictionColor** : définissez la couleur du caractère de début `>` et le nom de la source de fin, par exemple `[History]` . Par défaut, il utilise `DarkYellow` comme couleur de premier plan.
- **ListPredictionSelectedColor** -définit la couleur pour indiquer qu’un élément de liste est sélectionné. Par défaut, il utilise `DarkBlack` comme couleur d’arrière-plan.
...
- **Aucun** : désactive la fonctionnalité IntelliSense prédictive (par défaut).
...
Définit le style d’affichage du texte prédictif. La valeur par défaut est **InlineView**.

- **InlineView** : style actuel actuel, similaire à celui de poisson Shell et zsh. (par défaut)
- **ListView** : les suggestions s’affichent dans une liste déroulante, et les utilisateurs peuvent sélectionner en utilisant les <kbd>flèches haut</kbd> et <kbd>bas</kbd>.
