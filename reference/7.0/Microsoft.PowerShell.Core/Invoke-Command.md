...
### <span data-ttu-id="92b67-109">session</span><span class="sxs-lookup"><span data-stu-id="92b67-109">Session</span></span>
...
### <span data-ttu-id="92b67-119">ContainerId</span><span class="sxs-lookup"><span data-stu-id="92b67-119">ContainerId</span></span>
...
<span data-ttu-id="92b67-151">En réponse, PowerShell demande le mot de passe et une méthode d’authentification pour le compte User01.</span><span class="sxs-lookup"><span data-stu-id="92b67-151">In response, PowerShell requests the password and an authentication method for the User01 account.</span></span>
...
<span data-ttu-id="92b67-174">Cet exemple montre comment créer une commande stockée en tant que bloc de script dans une variable locale.</span><span class="sxs-lookup"><span data-stu-id="92b67-174">This example shows how to create a command that is stored as a script block in a local variable.</span></span>
...
### <span data-ttu-id="92b67-184">Exemple 7 : récupération de la version du programme hôte sur plusieurs ordinateurs</span><span class="sxs-lookup"><span data-stu-id="92b67-184">Example 7: Get the version of the host program on multiple computers</span></span>
...
### <span data-ttu-id="92b67-207">Exemple 9 : inclure des variables locales dans une commande exécutée sur un ordinateur distant</span><span class="sxs-lookup"><span data-stu-id="92b67-207">Example 9: Include local variables in a command run on a remote computer</span></span>
...
<span data-ttu-id="92b67-236">`Invoke-Command` utilise un paramètre **scriptblock** et `Get-ChildItem` spécifie `$args[0]` les `$args[1]` valeurs de tableau et.</span><span class="sxs-lookup"><span data-stu-id="92b67-236">`Invoke-Command` uses a **ScriptBlock** parameter and `Get-ChildItem` specifies the `$args[0]` and `$args[1]` array values.</span></span> <span data-ttu-id="92b67-237">Le **argumentlist** passe les `$args` valeurs de tableau aux `Get-ChildItem` positions de paramètre pour le **chemin d’accès** et le **filtre** .</span><span class="sxs-lookup"><span data-stu-id="92b67-237">The **ArgumentList** passes the `$args` array values to the `Get-ChildItem` parameter positions for **Path** and **Filter** .</span></span>
...
<span data-ttu-id="92b67-278">L' `Enable-WSManCredSSP` applet de commande active la délégation CredSSP à partir de l’ordinateur local SERVEUR01 sur l’ordinateur distant SERVER02.</span><span class="sxs-lookup"><span data-stu-id="92b67-278">The `Enable-WSManCredSSP` cmdlet enables CredSSP delegation from the Server01 local computer to the Server02 remote computer.</span></span> <span data-ttu-id="92b67-279">Le paramètre **role** spécifie au **client** de configurer le paramètre client CredSSP sur l’ordinateur local.</span><span class="sxs-lookup"><span data-stu-id="92b67-279">The **Role** parameter specifies **Client** to configure the CredSSP client setting on the local computer.</span></span>
...
### <span data-ttu-id="92b67-311">-AllowRedirection</span><span class="sxs-lookup"><span data-stu-id="92b67-311">-AllowRedirection</span></span>
...
<span data-ttu-id="92b67-362">Pour plus d’informations sur les valeurs de ce paramètre, consultez [énumération AuthenticationMechanism](/dotnet/api/system.management.automation.runspaces.authenticationmechanism).</span><span class="sxs-lookup"><span data-stu-id="92b67-362">For more information about the values of this parameter, see [AuthenticationMechanism Enumeration](/dotnet/api/system.management.automation.runspaces.authenticationmechanism).</span></span>
...
<span data-ttu-id="92b67-401">Les valeurs valides du segment **Transport** de l'URI sont HTTP et HTTPS.</span><span class="sxs-lookup"><span data-stu-id="92b67-401">Valid values for the **Transport** segment of the URI are HTTP and HTTPS.</span></span> <span data-ttu-id="92b67-402">Si vous spécifiez un URI de connexion avec un segment de transport, mais que vous ne spécifiez pas de port, la session est créée avec les ports standard : 80 pour HTTP et 443 pour HTTPs.</span><span class="sxs-lookup"><span data-stu-id="92b67-402">If you specify a connection URI with a Transport segment, but don't specify a port, the session is created with the standards ports: 80 for HTTP and 443 for HTTPS.</span></span> <span data-ttu-id="92b67-403">Pour utiliser les ports par défaut pour la communication à distance PowerShell, spécifiez le port 5985 pour HTTP ou 5986 pour HTTPs.</span><span class="sxs-lookup"><span data-stu-id="92b67-403">To use the default ports for PowerShell remoting, specify port 5985 for HTTP or 5986 for HTTPS.</span></span>

<span data-ttu-id="92b67-404">Si l’ordinateur de destination redirige la connexion vers un autre URI, PowerShell empêche la redirection, sauf si vous utilisez le paramètre **AllowRedirection** dans la commande.</span><span class="sxs-lookup"><span data-stu-id="92b67-404">If the destination computer redirects the connection to a different URI, PowerShell prevents the redirection unless you use the **AllowRedirection** parameter in the command.</span></span>
...
### <span data-ttu-id="92b67-405">-ContainerId</span><span class="sxs-lookup"><span data-stu-id="92b67-405">-ContainerId</span></span>

<span data-ttu-id="92b67-406">Spécifie un tableau d’ID de conteneur.</span><span class="sxs-lookup"><span data-stu-id="92b67-406">Specifies an array of container IDs.</span></span>
...
### <span data-ttu-id="92b67-407">-Credential</span><span class="sxs-lookup"><span data-stu-id="92b67-407">-Credential</span></span>

<span data-ttu-id="92b67-408">Spécifie un compte d’utilisateur qui a l’autorisation d’exécuter cette action.</span><span class="sxs-lookup"><span data-stu-id="92b67-408">Specifies a user account that has permission to perform this action.</span></span> <span data-ttu-id="92b67-409">La valeur par défaut est l’utilisateur actuel.</span><span class="sxs-lookup"><span data-stu-id="92b67-409">The default is the current user.</span></span>

<span data-ttu-id="92b67-410">Tapez un nom d’utilisateur, tel que **User01** ou **Domaine01\Utilisateur01** , ou entrez un objet **PSCredential** généré par l’applet de commande `Get-Credential` .</span><span class="sxs-lookup"><span data-stu-id="92b67-410">Type a user name, such as **User01** or **Domain01\User01** , or enter a **PSCredential** object generated by the `Get-Credential` cmdlet.</span></span> <span data-ttu-id="92b67-411">Si vous tapez un nom d’utilisateur, vous êtes invité à entrer le mot de passe.</span><span class="sxs-lookup"><span data-stu-id="92b67-411">If you type a user name, you're prompted to enter the password.</span></span>

<span data-ttu-id="92b67-412">Les informations d’identification sont stockées dans un objet [PSCredential](/dotnet/api/system.management.automation.pscredential) et le mot de passe est stocké en tant que [SecureString](/dotnet/api/system.security.securestring).</span><span class="sxs-lookup"><span data-stu-id="92b67-412">Credentials are stored in a [PSCredential](/dotnet/api/system.management.automation.pscredential) object and the password is stored as a [SecureString](/dotnet/api/system.security.securestring).</span></span>

> [!NOTE]
> <span data-ttu-id="92b67-413">Pour plus d’informations sur la protection des données **SecureString** , consultez la section relative à [la sécurité de SecureString](/dotnet/api/system.security.securestring#how-secure-is-securestring).</span><span class="sxs-lookup"><span data-stu-id="92b67-413">For more information about **SecureString** data protection, see [How secure is SecureString?](/dotnet/api/system.security.securestring#how-secure-is-securestring).</span></span>
...
### <span data-ttu-id="92b67-414">-EnableNetworkAccess</span><span class="sxs-lookup"><span data-stu-id="92b67-414">-EnableNetworkAccess</span></span>

<span data-ttu-id="92b67-415">Indique que cette applet de commande ajoute un jeton de sécurité interactif aux sessions de bouclage.</span><span class="sxs-lookup"><span data-stu-id="92b67-415">Indicates that this cmdlet adds an interactive security token to loopback sessions.</span></span> <span data-ttu-id="92b67-416">Le jeton interactif vous permet d'exécuter des commandes dans la session de bouclage qui obtiennent des données à partir d'autres ordinateurs.</span><span class="sxs-lookup"><span data-stu-id="92b67-416">The interactive token lets you run commands in the loopback session that get data from other computers.</span></span> <span data-ttu-id="92b67-417">Par exemple, vous pouvez exécuter une commande dans la session qui copie les fichiers XML d'un ordinateur distant vers l'ordinateur local.</span><span class="sxs-lookup"><span data-stu-id="92b67-417">For example, you can run a command in the session that copies XML files from a remote computer to the local computer.</span></span>

<span data-ttu-id="92b67-418">Une session de bouclage est une session **PSSession** qui provient et se termine sur le même ordinateur.</span><span class="sxs-lookup"><span data-stu-id="92b67-418">A loopback session is a **PSSession** that originates and ends on the same computer.</span></span> <span data-ttu-id="92b67-419">Pour créer une session de bouclage, omettez le paramètre **ComputerName** ou affectez-lui la valeur dot ( `.` ), localhost ou le nom de l’ordinateur local.</span><span class="sxs-lookup"><span data-stu-id="92b67-419">To create a loopback session, omit the **ComputerName** parameter or set its value to dot (`.`), localhost, or the name of the local computer.</span></span>

<span data-ttu-id="92b67-420">Par défaut, les sessions de bouclage sont créées à l’aide d’un jeton réseau, qui peut ne pas fournir les autorisations suffisantes pour s’authentifier auprès des ordinateurs distants.</span><span class="sxs-lookup"><span data-stu-id="92b67-420">By default, loopback sessions are created using a network token, which might not provide sufficient permission to authenticate to remote computers.</span></span>

<span data-ttu-id="92b67-421">Le paramètre **EnableNetworkAccess** n'est effectif que dans les sessions de bouclage.</span><span class="sxs-lookup"><span data-stu-id="92b67-421">The **EnableNetworkAccess** parameter is effective only in loopback sessions.</span></span> <span data-ttu-id="92b67-422">Si vous utilisez **EnableNetworkAccess** quand vous créez une session sur un ordinateur distant, la commande s’exécute correctement, mais le paramètre est ignoré.</span><span class="sxs-lookup"><span data-stu-id="92b67-422">If you use **EnableNetworkAccess** when you create a session on a remote computer, the command succeeds, but the parameter is ignored.</span></span>

<span data-ttu-id="92b67-423">Vous pouvez autoriser l’accès à distance dans une session de bouclage à l’aide de la valeur **CredSSP** du paramètre **Authentication** , qui délègue les informations d’identification de session à d’autres ordinateurs.</span><span class="sxs-lookup"><span data-stu-id="92b67-423">You can allow remote access in a loopback session using the **CredSSP** value of the **Authentication** parameter, which delegates the session credentials to other computers.</span></span>

<span data-ttu-id="92b67-424">Pour protéger l’ordinateur contre les accès malveillants, les sessions de bouclage déconnectées qui ont des jetons interactifs, qui sont créés à l’aide de **EnableNetworkAccess** , peuvent être reconnectées uniquement à partir de l’ordinateur sur lequel la session a été créée.</span><span class="sxs-lookup"><span data-stu-id="92b67-424">To protect the computer from malicious access, disconnected loopback sessions that have interactive tokens, which are those created using **EnableNetworkAccess** , can be reconnected only from the computer on which the session was created.</span></span> <span data-ttu-id="92b67-425">Les sessions déconnectées qui utilisent l'authentification CredSSP peuvent être reconnectées à partir d'autres ordinateurs.</span><span class="sxs-lookup"><span data-stu-id="92b67-425">Disconnected sessions that use CredSSP authentication can be reconnected from other computers.</span></span> <span data-ttu-id="92b67-426">Pour plus d'informations, consultez `Disconnect-PSSession`.</span><span class="sxs-lookup"><span data-stu-id="92b67-426">For more information, see `Disconnect-PSSession`.</span></span>

<span data-ttu-id="92b67-427">Ce paramètre a été introduit dans PowerShell 3,0.</span><span class="sxs-lookup"><span data-stu-id="92b67-427">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="92b67-428">-FilePath</span><span class="sxs-lookup"><span data-stu-id="92b67-428">-FilePath</span></span>

<span data-ttu-id="92b67-429">Spécifie un script local que cette applet de commande exécute sur un ou plusieurs ordinateurs distants.</span><span class="sxs-lookup"><span data-stu-id="92b67-429">Specifies a local script that this cmdlet runs on one or more remote computers.</span></span> <span data-ttu-id="92b67-430">Entrez le chemin d’accès et le nom de fichier du script, ou dirigez un chemin d’accès de script vers `Invoke-Command` .</span><span class="sxs-lookup"><span data-stu-id="92b67-430">Enter the path and filename of the script, or pipe a script path to `Invoke-Command`.</span></span> <span data-ttu-id="92b67-431">Le script doit exister sur l’ordinateur local ou dans un répertoire auquel l’ordinateur local peut accéder.</span><span class="sxs-lookup"><span data-stu-id="92b67-431">The script must exist on the local computer or in a directory that the local computer can access.</span></span> <span data-ttu-id="92b67-432">Utilisez **argumentlist** pour spécifier les valeurs des paramètres dans le script.</span><span class="sxs-lookup"><span data-stu-id="92b67-432">Use **ArgumentList** to specify the values of parameters in the script.</span></span>

<span data-ttu-id="92b67-433">Lorsque vous utilisez ce paramètre, PowerShell convertit le contenu du fichier de script spécifié en un bloc de script, transmet le bloc de script à l’ordinateur distant et l’exécute sur l’ordinateur distant.</span><span class="sxs-lookup"><span data-stu-id="92b67-433">When you use this parameter, PowerShell converts the contents of the specified script file to a script block, transmits the script block to the remote computer, and runs it on the remote computer.</span></span>
...
### <span data-ttu-id="92b67-434">-HideComputerName</span><span class="sxs-lookup"><span data-stu-id="92b67-434">-HideComputerName</span></span>

<span data-ttu-id="92b67-435">Indique que cette applet de commande omet le nom d’ordinateur de chaque objet dans l’affichage de sortie.</span><span class="sxs-lookup"><span data-stu-id="92b67-435">Indicates that this cmdlet omits the computer name of each object from the output display.</span></span> <span data-ttu-id="92b67-436">Par défaut, le nom de l'ordinateur qui a généré l'objet apparaît dans l'affichage.</span><span class="sxs-lookup"><span data-stu-id="92b67-436">By default, the name of the computer that generated the object appears in the display.</span></span>

<span data-ttu-id="92b67-437">Ce paramètre affecte uniquement l'affichage de la sortie.</span><span class="sxs-lookup"><span data-stu-id="92b67-437">This parameter affects only the output display.</span></span> <span data-ttu-id="92b67-438">Elle ne modifie pas l’objet.</span><span class="sxs-lookup"><span data-stu-id="92b67-438">It doesn't change the object.</span></span>
...
### <span data-ttu-id="92b67-439">-HostName</span><span class="sxs-lookup"><span data-stu-id="92b67-439">-HostName</span></span>

<span data-ttu-id="92b67-440">Spécifie un tableau de noms d’ordinateurs pour une connexion Secure Shell (SSH).</span><span class="sxs-lookup"><span data-stu-id="92b67-440">Specifies an array of computer names for a Secure Shell (SSH) based connection.</span></span> <span data-ttu-id="92b67-441">Cela est similaire au paramètre **ComputerName** , à ceci près que la connexion à l’ordinateur distant s’effectue à l’aide de SSH plutôt que de Windows WinRM.</span><span class="sxs-lookup"><span data-stu-id="92b67-441">This is similar to the **ComputerName** parameter except that the connection to the remote computer is made using SSH rather than Windows WinRM.</span></span>

<span data-ttu-id="92b67-442">Ce paramètre a été introduit dans PowerShell 6,0.</span><span class="sxs-lookup"><span data-stu-id="92b67-442">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="92b67-443">-InDisconnectedSession</span><span class="sxs-lookup"><span data-stu-id="92b67-443">-InDisconnectedSession</span></span>

<span data-ttu-id="92b67-444">Indique que cette applet de commande exécute une commande ou un script dans une session déconnectée.</span><span class="sxs-lookup"><span data-stu-id="92b67-444">Indicates that this cmdlet runs a command or script in a disconnected session.</span></span>

<span data-ttu-id="92b67-445">Quand vous utilisez le paramètre **InDisconnectedSession** , `Invoke-Command` crée une session persistante sur chaque ordinateur distant, démarre la commande spécifiée par le paramètre **scriptblock** ou **filePath** , puis se déconnecte de la session.</span><span class="sxs-lookup"><span data-stu-id="92b67-445">When you use the **InDisconnectedSession** parameter, `Invoke-Command` creates a persistent session on each remote computer, starts the command specified by the **ScriptBlock** or **FilePath** parameter, and then disconnects from the session.</span></span> <span data-ttu-id="92b67-446">Les commandes continuent à s’exécuter dans les sessions déconnectées.</span><span class="sxs-lookup"><span data-stu-id="92b67-446">The commands continue to run in the disconnected sessions.</span></span> <span data-ttu-id="92b67-447">**InDisconnectedSession** vous permet d’exécuter des commandes sans maintenir une connexion aux sessions à distance.</span><span class="sxs-lookup"><span data-stu-id="92b67-447">**InDisconnectedSession** enables you to run commands without maintaining a connection to the remote sessions.</span></span> <span data-ttu-id="92b67-448">Et, étant donné que la session est déconnectée avant le renvoi des résultats, **InDisconnectedSession** garantit que tous les résultats de la commande sont retournés à la session reconnectée, au lieu d’être fractionnés entre les sessions.</span><span class="sxs-lookup"><span data-stu-id="92b67-448">And, because the session is disconnected before any results are returned, **InDisconnectedSession** makes sure that all command results are returned to the reconnected session, instead of being split between sessions.</span></span>

<span data-ttu-id="92b67-449">Vous ne pouvez pas utiliser **InDisconnectedSession** avec le paramètre de **session** ou le paramètre **AsJob** .</span><span class="sxs-lookup"><span data-stu-id="92b67-449">You can't use **InDisconnectedSession** with the **Session** parameter or the **AsJob** parameter.</span></span>

<span data-ttu-id="92b67-450">Les commandes qui utilisent **InDisconnectedSession** retournent un objet **PSSession** qui représente la session déconnectée.</span><span class="sxs-lookup"><span data-stu-id="92b67-450">Commands that use **InDisconnectedSession** return a **PSSession** object that represents the disconnected session.</span></span> <span data-ttu-id="92b67-451">Ils ne retournent pas la sortie de commande.</span><span class="sxs-lookup"><span data-stu-id="92b67-451">They don't return the command output.</span></span> <span data-ttu-id="92b67-452">Pour vous connecter à la session déconnectée, utilisez les `Connect-PSSession` applets de commande ou `Receive-PSSession` .</span><span class="sxs-lookup"><span data-stu-id="92b67-452">To connect to the disconnected session, use the `Connect-PSSession` or `Receive-PSSession` cmdlets.</span></span> <span data-ttu-id="92b67-453">Pour obtenir les résultats des commandes exécutées dans la session, utilisez l' `Receive-PSSession` applet de commande.</span><span class="sxs-lookup"><span data-stu-id="92b67-453">To get the results of commands that ran in the session, use the `Receive-PSSession` cmdlet.</span></span> <span data-ttu-id="92b67-454">Pour exécuter des commandes qui génèrent une sortie dans une session déconnectée, définissez la valeur de l’option de session **OutputBufferingMode** sur **Drop** .</span><span class="sxs-lookup"><span data-stu-id="92b67-454">To run commands that generate output in a disconnected session, set the value of the **OutputBufferingMode** session option to **Drop** .</span></span> <span data-ttu-id="92b67-455">Si vous envisagez de vous connecter à la session déconnectée, définissez le délai d’inactivité dans la session afin qu’elle fournisse suffisamment de temps pour vous connecter avant de supprimer la session.</span><span class="sxs-lookup"><span data-stu-id="92b67-455">If you intend to connect to the disconnected session, set the idle time-out in the session so that it provides sufficient time for you to connect before deleting the session.</span></span>

<span data-ttu-id="92b67-456">Vous pouvez définir le mode de mise en mémoire tampon de sortie et le délai d’inactivité dans le paramètre **SessionOption** ou dans la `$PSSessionOption` variable de préférence.</span><span class="sxs-lookup"><span data-stu-id="92b67-456">You can set the output buffering mode and idle time-out in the **SessionOption** parameter or in the `$PSSessionOption` preference variable.</span></span> <span data-ttu-id="92b67-457">Pour plus d’informations sur les options de session, consultez `New-PSSessionOption` et [about_Preference_Variables](./about/about_preference_variables.md).</span><span class="sxs-lookup"><span data-stu-id="92b67-457">For more information about session options, see `New-PSSessionOption` and [about_Preference_Variables](./about/about_preference_variables.md).</span></span>

<span data-ttu-id="92b67-458">Pour plus d'informations sur la fonctionnalité Sessions déconnectées, consultez [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md).</span><span class="sxs-lookup"><span data-stu-id="92b67-458">For more information about the Disconnected Sessions feature, see [about_Remote_Disconnected_Sessions](about/about_Remote_Disconnected_Sessions.md).</span></span>

<span data-ttu-id="92b67-459">Ce paramètre a été introduit dans PowerShell 3,0.</span><span class="sxs-lookup"><span data-stu-id="92b67-459">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="92b67-460">-InputObject</span><span class="sxs-lookup"><span data-stu-id="92b67-460">-InputObject</span></span>

<span data-ttu-id="92b67-461">Spécifie l'entrée de la commande.</span><span class="sxs-lookup"><span data-stu-id="92b67-461">Specifies input to the command.</span></span> <span data-ttu-id="92b67-462">Entrez une variable contenant les objets, ou tapez une commande ou une expression qui les obtient.</span><span class="sxs-lookup"><span data-stu-id="92b67-462">Enter a variable that contains the objects or type a command or expression that gets the objects.</span></span>

<span data-ttu-id="92b67-463">Quand vous utilisez le paramètre **InputObject** , utilisez la `$Input` variable Automatic dans la valeur du paramètre **scriptblock** pour représenter les objets d’entrée.</span><span class="sxs-lookup"><span data-stu-id="92b67-463">When using the **InputObject** parameter, use the `$Input` automatic variable in the value of the **ScriptBlock** parameter to represent the input objects.</span></span>
...
### <span data-ttu-id="92b67-464">-JobName</span><span class="sxs-lookup"><span data-stu-id="92b67-464">-JobName</span></span>

<span data-ttu-id="92b67-465">Spécifie un nom convivial pour la tâche en arrière-plan.</span><span class="sxs-lookup"><span data-stu-id="92b67-465">Specifies a friendly name for the background job.</span></span> <span data-ttu-id="92b67-466">Par défaut, les tâches sont nommées `Job<n>` , où `<n>` est un nombre ordinal.</span><span class="sxs-lookup"><span data-stu-id="92b67-466">By default, jobs are named `Job<n>`, where `<n>` is an ordinal number.</span></span>

<span data-ttu-id="92b67-467">Si vous utilisez le paramètre **JobName** dans une commande, la commande est exécutée en tant que tâche et `Invoke-Command` retourne un objet de traitement, même si vous n’incluez pas **AsJob** dans la commande.</span><span class="sxs-lookup"><span data-stu-id="92b67-467">If you use the **JobName** parameter in a command, the command is run as a job, and `Invoke-Command` returns a job object, even if you don't include **AsJob** in the command.</span></span>

<span data-ttu-id="92b67-468">Pour plus d’informations sur les travaux en arrière-plan PowerShell, consultez [about_Jobs](./About/about_Jobs.md).</span><span class="sxs-lookup"><span data-stu-id="92b67-468">For more information about PowerShell background jobs, see [about_Jobs](./About/about_Jobs.md).</span></span>
...
### <span data-ttu-id="92b67-469">-KeyFilePath</span><span class="sxs-lookup"><span data-stu-id="92b67-469">-KeyFilePath</span></span>

<span data-ttu-id="92b67-470">Spécifie un chemin de fichier de clé utilisé par Secure Shell (SSH) pour authentifier un utilisateur sur un ordinateur distant.</span><span class="sxs-lookup"><span data-stu-id="92b67-470">Specifies a key file path used by Secure Shell (SSH) to authenticate a user on a remote computer.</span></span>

<span data-ttu-id="92b67-471">SSH permet d’effectuer l’authentification des utilisateurs via des clés privées et publiques comme alternative à l’authentification de base par mot de passe.</span><span class="sxs-lookup"><span data-stu-id="92b67-471">SSH allows user authentication to be performed via private and public keys as an alternative to basic password authentication.</span></span> <span data-ttu-id="92b67-472">Si l’ordinateur distant est configuré pour l’authentification de clé, ce paramètre peut être utilisé pour fournir la clé qui identifie l’utilisateur.</span><span class="sxs-lookup"><span data-stu-id="92b67-472">If the remote computer is configured for key authentication, then this parameter can be used to provide the key that identifies the user.</span></span>

<span data-ttu-id="92b67-473">Ce paramètre a été introduit dans PowerShell 6,0.</span><span class="sxs-lookup"><span data-stu-id="92b67-473">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="92b67-474">-NoNewScope</span><span class="sxs-lookup"><span data-stu-id="92b67-474">-NoNewScope</span></span>

<span data-ttu-id="92b67-475">Indique que cette applet de commande exécute la commande spécifiée dans l’étendue actuelle.</span><span class="sxs-lookup"><span data-stu-id="92b67-475">Indicates that this cmdlet runs the specified command in the current scope.</span></span> <span data-ttu-id="92b67-476">Par défaut, `Invoke-Command` exécute des commandes dans leur propre étendue.</span><span class="sxs-lookup"><span data-stu-id="92b67-476">By default, `Invoke-Command` runs commands in their own scope.</span></span>

<span data-ttu-id="92b67-477">Ce paramètre est valide uniquement dans les commandes qui sont exécutées dans la session active, autrement dit, dans les commandes qui omettent les deux paramètres **ComputerName** et **Session** .</span><span class="sxs-lookup"><span data-stu-id="92b67-477">This parameter is valid only in commands that are run in the current session, that is, commands that omit both the **ComputerName** and **Session** parameters.</span></span>

<span data-ttu-id="92b67-478">Ce paramètre a été introduit dans PowerShell 3,0.</span><span class="sxs-lookup"><span data-stu-id="92b67-478">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="92b67-479">-Port</span><span class="sxs-lookup"><span data-stu-id="92b67-479">-Port</span></span>

<span data-ttu-id="92b67-480">Spécifie le port réseau sur l’ordinateur distant utilisé pour cette commande.</span><span class="sxs-lookup"><span data-stu-id="92b67-480">Specifies the network port on the remote computer that is used for this command.</span></span> <span data-ttu-id="92b67-481">Pour établir une connexion à un ordinateur distant, l’ordinateur distant doit être à l’écoute sur le port utilisé par la connexion.</span><span class="sxs-lookup"><span data-stu-id="92b67-481">To connect to a remote computer, the remote computer must be listening on the port that the connection uses.</span></span> <span data-ttu-id="92b67-482">Les ports par défaut sont 5985 (port WinRM pour HTTP) et 5986 (port WinRM pour HTTPs).</span><span class="sxs-lookup"><span data-stu-id="92b67-482">The default ports are 5985 (WinRM port for HTTP) and 5986 (WinRM port for HTTPS).</span></span>

<span data-ttu-id="92b67-483">Avant d'utiliser un autre port, configurez l'écouteur WinRM sur l'ordinateur distant pour qu'il écoute sur ce port.</span><span class="sxs-lookup"><span data-stu-id="92b67-483">Before using an alternate port, configure the WinRM listener on the remote computer to listen at that port.</span></span> <span data-ttu-id="92b67-484">Pour configurer l’écouteur, tapez les deux commandes suivantes à l’invite de PowerShell :</span><span class="sxs-lookup"><span data-stu-id="92b67-484">To configure the listener, type the following two commands at the PowerShell prompt:</span></span>

`Remove-Item -Path WSMan:\Localhost\listener\listener* -Recurse`

`New-Item -Path WSMan:\Localhost\listener -Transport http -Address * -Port \<port-number\>`

<span data-ttu-id="92b67-485">N’utilisez pas le paramètre **port** , sauf si vous devez le faire.</span><span class="sxs-lookup"><span data-stu-id="92b67-485">Don't use the **Port** parameter unless you must.</span></span> <span data-ttu-id="92b67-486">Le port défini dans la commande s'applique à tous les ordinateurs ou toutes les sessions sur lesquelles la commande s'exécute.</span><span class="sxs-lookup"><span data-stu-id="92b67-486">The port that is set in the command applies to all computers or sessions on which the command runs.</span></span> <span data-ttu-id="92b67-487">Un autre paramètre de port peut empêcher la commande de s'exécuter sur tous les ordinateurs.</span><span class="sxs-lookup"><span data-stu-id="92b67-487">An alternate port setting might prevent the command from running on all computers.</span></span>
...
### <span data-ttu-id="92b67-488">-RemoteDebug</span><span class="sxs-lookup"><span data-stu-id="92b67-488">-RemoteDebug</span></span>

<span data-ttu-id="92b67-489">Utilisé pour exécuter la commande appelée en mode débogage dans la session PowerShell distante.</span><span class="sxs-lookup"><span data-stu-id="92b67-489">Used to run the invoked command in debug mode in the remote PowerShell session.</span></span>
...
### <span data-ttu-id="92b67-490">-RunAsAdministrator</span><span class="sxs-lookup"><span data-stu-id="92b67-490">-RunAsAdministrator</span></span>

<span data-ttu-id="92b67-491">Indique que cette applet de commande appelle une commande en tant qu’administrateur.</span><span class="sxs-lookup"><span data-stu-id="92b67-491">Indicates that this cmdlet invokes a command as an Administrator.</span></span>
...
### <span data-ttu-id="92b67-492">-ScriptBlock</span><span class="sxs-lookup"><span data-stu-id="92b67-492">-ScriptBlock</span></span>

<span data-ttu-id="92b67-493">Spécifie les commandes à exécuter.</span><span class="sxs-lookup"><span data-stu-id="92b67-493">Specifies the commands to run.</span></span> <span data-ttu-id="92b67-494">Placez les commandes entre accolades `{ }` pour créer un bloc de script.</span><span class="sxs-lookup"><span data-stu-id="92b67-494">Enclose the commands in curly braces `{ }` to create a script block.</span></span>
<span data-ttu-id="92b67-495">Ce paramètre est obligatoire.</span><span class="sxs-lookup"><span data-stu-id="92b67-495">This parameter is required.</span></span>

<span data-ttu-id="92b67-496">Par défaut, les variables de la commande sont évaluées sur l'ordinateur distant.</span><span class="sxs-lookup"><span data-stu-id="92b67-496">By default, any variables in the command are evaluated on the remote computer.</span></span> <span data-ttu-id="92b67-497">Pour inclure des variables locales dans la commande, utilisez **argumentlist** .</span><span class="sxs-lookup"><span data-stu-id="92b67-497">To include local variables in the command, use **ArgumentList** .</span></span>
...
### <span data-ttu-id="92b67-498">-Session</span><span class="sxs-lookup"><span data-stu-id="92b67-498">-Session</span></span>

<span data-ttu-id="92b67-499">Spécifie un tableau de sessions dans lequel cette applet de commande exécute la commande.</span><span class="sxs-lookup"><span data-stu-id="92b67-499">Specifies an array of sessions in which this cmdlet runs the command.</span></span> <span data-ttu-id="92b67-500">Entrez une variable qui contient des objets **PSSession** ou une commande qui crée ou obtient les objets **PSSession** , tels qu' `New-PSSession` une `Get-PSSession` commande ou.</span><span class="sxs-lookup"><span data-stu-id="92b67-500">Enter a variable that contains **PSSession** objects or a command that creates or gets the **PSSession** objects, such as a `New-PSSession` or `Get-PSSession` command.</span></span>

<span data-ttu-id="92b67-501">Lorsque vous créez une **session PSSession** , PowerShell établit une connexion permanente à l’ordinateur distant.</span><span class="sxs-lookup"><span data-stu-id="92b67-501">When you create a **PSSession** , PowerShell establishes a persistent connection to the remote computer.</span></span> <span data-ttu-id="92b67-502">Utilisez une **session PSSession** pour exécuter une série de commandes associées qui partagent des données.</span><span class="sxs-lookup"><span data-stu-id="92b67-502">Use a **PSSession** to run a series of related commands that share data.</span></span> <span data-ttu-id="92b67-503">Pour exécuter une seule commande ou une série de commandes sans rapport, utilisez le paramètre **ComputerName** .</span><span class="sxs-lookup"><span data-stu-id="92b67-503">To run a single command or a series of unrelated commands, use the **ComputerName** parameter.</span></span> <span data-ttu-id="92b67-504">Pour plus d’informations, consultez [about_PSSessions](./About/about_PSSessions.md).</span><span class="sxs-lookup"><span data-stu-id="92b67-504">For more information, see [about_PSSessions](./About/about_PSSessions.md).</span></span>
...
### <span data-ttu-id="92b67-505">-Nom_session</span><span class="sxs-lookup"><span data-stu-id="92b67-505">-SessionName</span></span>

<span data-ttu-id="92b67-506">Spécifie un nom convivial pour une session déconnectée.</span><span class="sxs-lookup"><span data-stu-id="92b67-506">Specifies a friendly name for a disconnected session.</span></span> <span data-ttu-id="92b67-507">Vous pouvez utiliser le nom pour faire référence à la session dans les commandes suivantes, par exemple une `Get-PSSession` commande.</span><span class="sxs-lookup"><span data-stu-id="92b67-507">You can use the name to refer to the session in subsequent commands, such as a `Get-PSSession` command.</span></span> <span data-ttu-id="92b67-508">Ce paramètre est valide uniquement avec le paramètre **InDisconnectedSession** .</span><span class="sxs-lookup"><span data-stu-id="92b67-508">This parameter is valid only with the **InDisconnectedSession** parameter.</span></span>

<span data-ttu-id="92b67-509">Ce paramètre a été introduit dans PowerShell 3,0.</span><span class="sxs-lookup"><span data-stu-id="92b67-509">This parameter was introduced in PowerShell 3.0.</span></span>
...
### <span data-ttu-id="92b67-510">-SessionOption</span><span class="sxs-lookup"><span data-stu-id="92b67-510">-SessionOption</span></span>

<span data-ttu-id="92b67-511">Spécifie les options avancées pour la session.</span><span class="sxs-lookup"><span data-stu-id="92b67-511">Specifies advanced options for the session.</span></span> <span data-ttu-id="92b67-512">Entrez un objet **SessionOption** , tel que celui que vous créez à l’aide de l' `New-PSSessionOption` applet de commande, ou une table de hachage dans laquelle les clés sont des noms d’option de session et les valeurs sont des valeurs d’option de session.</span><span class="sxs-lookup"><span data-stu-id="92b67-512">Enter a **SessionOption** object, such as one that you create using the `New-PSSessionOption` cmdlet, or a hash table in which the keys are session option names and the values are session option values.</span></span>

<span data-ttu-id="92b67-513">Les valeurs par défaut des options sont déterminées par la valeur de la `$PSSessionOption` variable de préférence, si elle est définie.</span><span class="sxs-lookup"><span data-stu-id="92b67-513">The default values for the options are determined by the value of the `$PSSessionOption` preference variable, if it's set.</span></span> <span data-ttu-id="92b67-514">Sinon, les valeurs par défaut sont établies par les options définies dans la configuration de session.</span><span class="sxs-lookup"><span data-stu-id="92b67-514">Otherwise, the default values are established by options set in the session configuration.</span></span>

<span data-ttu-id="92b67-515">Les valeurs d’option de session ont priorité sur les valeurs par défaut des sessions définies dans la `$PSSessionOption` variable de préférence et dans la configuration de session.</span><span class="sxs-lookup"><span data-stu-id="92b67-515">The session option values take precedence over default values for sessions set in the `$PSSessionOption` preference variable and in the session configuration.</span></span> <span data-ttu-id="92b67-516">Toutefois, elles ne sont pas prioritaires sur les valeurs maximales, les quotas ou les limites définies dans la configuration de session.</span><span class="sxs-lookup"><span data-stu-id="92b67-516">However, they don't take precedence over maximum values, quotas, or limits set in the session configuration.</span></span>

<span data-ttu-id="92b67-517">Pour obtenir une description des options de session qui incluent les valeurs par défaut, consultez `New-PSSessionOption` .</span><span class="sxs-lookup"><span data-stu-id="92b67-517">For a description of the session options that includes the default values, see `New-PSSessionOption`.</span></span> <span data-ttu-id="92b67-518">Pour plus d’informations sur la `$PSSessionOption` variable de préférence, consultez [about_Preference_Variables](About/about_Preference_Variables.md).</span><span class="sxs-lookup"><span data-stu-id="92b67-518">For information about the `$PSSessionOption` preference variable, see [about_Preference_Variables](About/about_Preference_Variables.md).</span></span>
<span data-ttu-id="92b67-519">Pour plus d’informations sur les configurations de session, consultez [about_session_configurations](About/about_Session_Configurations.md).</span><span class="sxs-lookup"><span data-stu-id="92b67-519">For more information about session configurations, see [about_Session_Configurations](About/about_Session_Configurations.md).</span></span>
...
### <span data-ttu-id="92b67-520">-SSHConnection</span><span class="sxs-lookup"><span data-stu-id="92b67-520">-SSHConnection</span></span>

<span data-ttu-id="92b67-521">Ce paramètre prend un tableau de tables de hachage dans lequel chaque table de hachage contient un ou plusieurs paramètres de connexion nécessaires pour établir une connexion Secure Shell (SSH).</span><span class="sxs-lookup"><span data-stu-id="92b67-521">This parameter takes an array of hash tables where each hash table contains one or more connection parameters needed to establish a Secure Shell (SSH) connection.</span></span> <span data-ttu-id="92b67-522">Le paramètre **SSHConnection** est utile pour créer plusieurs sessions où chaque session requiert des informations de connexion différentes.</span><span class="sxs-lookup"><span data-stu-id="92b67-522">The **SSHConnection** parameter is useful for creating multiple sessions where each session requires different connection information.</span></span>

<span data-ttu-id="92b67-523">La Hashtable contient les membres suivants :</span><span class="sxs-lookup"><span data-stu-id="92b67-523">The hashtable has the following members:</span></span>

- <span data-ttu-id="92b67-524">**ComputerName** (ou **hostname** )</span><span class="sxs-lookup"><span data-stu-id="92b67-524">**ComputerName** (or **HostName** )</span></span>
- <span data-ttu-id="92b67-525">**Port**</span><span class="sxs-lookup"><span data-stu-id="92b67-525">**Port**</span></span>
- <span data-ttu-id="92b67-526">**UserName**</span><span class="sxs-lookup"><span data-stu-id="92b67-526">**UserName**</span></span>
- <span data-ttu-id="92b67-527">**KeyFilePath** (ou **IdentityFilePath** )</span><span class="sxs-lookup"><span data-stu-id="92b67-527">**KeyFilePath** (or **IdentityFilePath** )</span></span>

<span data-ttu-id="92b67-528">**ComputerName** (ou **hostname** ) est la seule paire clé-valeur requise.</span><span class="sxs-lookup"><span data-stu-id="92b67-528">**ComputerName** (or **HostName** ) is the only key-value pair that is required.</span></span>

<span data-ttu-id="92b67-529">Ce paramètre a été introduit dans PowerShell 6,0.</span><span class="sxs-lookup"><span data-stu-id="92b67-529">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="92b67-530">-SSHTransport</span><span class="sxs-lookup"><span data-stu-id="92b67-530">-SSHTransport</span></span>

<span data-ttu-id="92b67-531">Indique que la connexion à distance est établie à l’aide d’Secure Shell (SSH).</span><span class="sxs-lookup"><span data-stu-id="92b67-531">Indicates that the remote connection is established using Secure Shell (SSH).</span></span>

<span data-ttu-id="92b67-532">Par défaut, PowerShell utilise Windows WinRM pour se connecter à un ordinateur distant.</span><span class="sxs-lookup"><span data-stu-id="92b67-532">By default PowerShell uses Windows WinRM to connect to a remote computer.</span></span> <span data-ttu-id="92b67-533">Ce commutateur force PowerShell à utiliser le paramètre **hostname** pour établir une connexion à distance basée sur SSH.</span><span class="sxs-lookup"><span data-stu-id="92b67-533">This switch forces PowerShell to use the **HostName** parameter for establishing an SSH based remote connection.</span></span>

<span data-ttu-id="92b67-534">Ce paramètre a été introduit dans PowerShell 6,0.</span><span class="sxs-lookup"><span data-stu-id="92b67-534">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="92b67-535">-ThrottleLimit</span><span class="sxs-lookup"><span data-stu-id="92b67-535">-ThrottleLimit</span></span>

<span data-ttu-id="92b67-536">Spécifie le nombre maximal de connexions simultanées qui peuvent être établies pour exécuter cette commande.</span><span class="sxs-lookup"><span data-stu-id="92b67-536">Specifies the maximum number of concurrent connections that can be established to run this command.</span></span>
<span data-ttu-id="92b67-537">Si vous omettez ce paramètre ou entrez la valeur 0, la valeur par défaut 32 est utilisée.</span><span class="sxs-lookup"><span data-stu-id="92b67-537">If you omit this parameter or enter a value of 0, the default value, 32, is used.</span></span>

<span data-ttu-id="92b67-538">La limite d'accélération s'applique uniquement à la commande actuelle, et non à la session ou à l'ordinateur.</span><span class="sxs-lookup"><span data-stu-id="92b67-538">The throttle limit applies only to the current command, not to the session or to the computer.</span></span>
...
### <span data-ttu-id="92b67-539">-UserName</span><span class="sxs-lookup"><span data-stu-id="92b67-539">-UserName</span></span>

<span data-ttu-id="92b67-540">Spécifie le nom d’utilisateur du compte utilisé pour exécuter une commande sur l’ordinateur distant.</span><span class="sxs-lookup"><span data-stu-id="92b67-540">Specifies the username for the account used to run a command on the remote computer.</span></span> <span data-ttu-id="92b67-541">La méthode d’authentification de l’utilisateur dépend de la configuration de Secure Shell (SSH) sur l’ordinateur distant.</span><span class="sxs-lookup"><span data-stu-id="92b67-541">The user authentication method depends on how Secure Shell (SSH) is configured on the remote computer.</span></span>

<span data-ttu-id="92b67-542">Si SSH est configuré pour l’authentification de mot de passe de base, vous êtes invité à entrer le mot de passe de l’utilisateur.</span><span class="sxs-lookup"><span data-stu-id="92b67-542">If SSH is configured for basic password authentication, then you'll be prompted for the user password.</span></span>

<span data-ttu-id="92b67-543">Si SSH est configuré pour l’authentification utilisateur basée sur les clés, un chemin d’accès de fichier de clé peut être fourni via le paramètre **keyFilePath** et aucune invite de mot de passe n’est générée.</span><span class="sxs-lookup"><span data-stu-id="92b67-543">If SSH is configured for key-based user authentication then a key file path can be provided via the **KeyFilePath** parameter and no password prompt occurs.</span></span> <span data-ttu-id="92b67-544">Si le fichier de clé de l’utilisateur client se trouve dans un emplacement connu SSH, le paramètre **keyFilePath** n’est pas nécessaire pour l’authentification basée sur les clés et l’authentification de l’utilisateur se produit automatiquement en fonction du nom d’utilisateur.</span><span class="sxs-lookup"><span data-stu-id="92b67-544">If the client user key file is located in an SSH known location, then the **KeyFilePath** parameter isn't needed for key-based authentication, and user authentication occurs automatically based on the username.</span></span> <span data-ttu-id="92b67-545">Pour plus d’informations, consultez la documentation SSH de votre plateforme relative à l’authentification des utilisateurs basée sur les clés.</span><span class="sxs-lookup"><span data-stu-id="92b67-545">For more information, see your platform's SSH documentation about key-based user authentication.</span></span>

<span data-ttu-id="92b67-546">Ce paramètre n’est pas obligatoire.</span><span class="sxs-lookup"><span data-stu-id="92b67-546">This isn't a required parameter.</span></span> <span data-ttu-id="92b67-547">Si le paramètre **username** n’est pas spécifié, le nom d’utilisateur actuellement connecté est utilisé pour la connexion.</span><span class="sxs-lookup"><span data-stu-id="92b67-547">If the **UserName** parameter isn't specified, then the current logged on username is used for the connection.</span></span>

<span data-ttu-id="92b67-548">Ce paramètre a été introduit dans PowerShell 6,0.</span><span class="sxs-lookup"><span data-stu-id="92b67-548">This parameter was introduced in PowerShell 6.0.</span></span>
...
### <span data-ttu-id="92b67-549">-UseSSL</span><span class="sxs-lookup"><span data-stu-id="92b67-549">-UseSSL</span></span>

<span data-ttu-id="92b67-550">Indique que cette applet de commande utilise le protocole protocole SSL (SSL) pour établir une connexion à l’ordinateur distant.</span><span class="sxs-lookup"><span data-stu-id="92b67-550">Indicates that this cmdlet uses the Secure Sockets Layer (SSL) protocol to establish a connection to the remote computer.</span></span> <span data-ttu-id="92b67-551">Par défaut, SSL n’est pas utilisé.</span><span class="sxs-lookup"><span data-stu-id="92b67-551">By default, SSL isn't used.</span></span>

<span data-ttu-id="92b67-552">WS-Management chiffre tout le contenu PowerShell transmis sur le réseau.</span><span class="sxs-lookup"><span data-stu-id="92b67-552">WS-Management encrypts all PowerShell content transmitted over the network.</span></span> <span data-ttu-id="92b67-553">Le paramètre **UseSSL** est une protection supplémentaire qui envoie les données via HTTPS au lieu de http.</span><span class="sxs-lookup"><span data-stu-id="92b67-553">The **UseSSL** parameter is an additional protection that sends the data across an HTTPS, instead of HTTP.</span></span>

<span data-ttu-id="92b67-554">Si vous utilisez ce paramètre, mais que SSL n’est pas disponible sur le port utilisé pour la commande, la commande échoue.</span><span class="sxs-lookup"><span data-stu-id="92b67-554">If you use this parameter, but SSL isn't available on the port that's used for the command, the command fails.</span></span>
...
### <span data-ttu-id="92b67-555">-VMId</span><span class="sxs-lookup"><span data-stu-id="92b67-555">-VMId</span></span>

<span data-ttu-id="92b67-556">Spécifie un tableau d’ID de machines virtuelles.</span><span class="sxs-lookup"><span data-stu-id="92b67-556">Specifies an array of IDs of virtual machines.</span></span>
...
### <span data-ttu-id="92b67-557">-VMName</span><span class="sxs-lookup"><span data-stu-id="92b67-557">-VMName</span></span>

<span data-ttu-id="92b67-558">Spécifie un tableau de noms d'ordinateurs virtuels.</span><span class="sxs-lookup"><span data-stu-id="92b67-558">Specifies an array of names of virtual machines.</span></span>
...
### <span data-ttu-id="92b67-559">-Sous-système</span><span class="sxs-lookup"><span data-stu-id="92b67-559">-Subsystem</span></span>

<span data-ttu-id="92b67-560">Spécifie le sous-système SSH utilisé pour la nouvelle **session PSSession** .</span><span class="sxs-lookup"><span data-stu-id="92b67-560">Specifies the SSH subsystem used for the new **PSSession** .</span></span>

<span data-ttu-id="92b67-561">Cela spécifie le sous-système à utiliser sur la cible, tel que défini dans sshd_config.</span><span class="sxs-lookup"><span data-stu-id="92b67-561">This specifies the subsystem to use on the target as defined in sshd_config.</span></span>
<span data-ttu-id="92b67-562">Le sous-système démarre une version spécifique de PowerShell avec des paramètres prédéfinis.</span><span class="sxs-lookup"><span data-stu-id="92b67-562">The subsystem starts a specific version of PowerShell with predefined parameters.</span></span>
<span data-ttu-id="92b67-563">Si le sous-système spécifié n’existe pas sur l’ordinateur distant, la commande échoue.</span><span class="sxs-lookup"><span data-stu-id="92b67-563">If the specified subsystem does not exist on the remote computer, the command fails.</span></span>

<span data-ttu-id="92b67-564">Si ce paramètre n’est pas utilisé, la valeur par défaut est le sous-système « PowerShell ».</span><span class="sxs-lookup"><span data-stu-id="92b67-564">If this parameter is not used, the default is the 'powershell' subsystem.</span></span>
...
### <span data-ttu-id="92b67-565">CommonParameters</span><span class="sxs-lookup"><span data-stu-id="92b67-565">CommonParameters</span></span>

<span data-ttu-id="92b67-566">Cette applet de commande prend en charge les paramètres courants : -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction et -WarningVariable.</span><span class="sxs-lookup"><span data-stu-id="92b67-566">This cmdlet supports the common parameters: -Debug, -ErrorAction, -ErrorVariable, -InformationAction, -InformationVariable, -OutVariable, -OutBuffer, -PipelineVariable, -Verbose, -WarningAction, and -WarningVariable.</span></span> <span data-ttu-id="92b67-567">Pour plus d’informations, consultez [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216).</span><span class="sxs-lookup"><span data-stu-id="92b67-567">For more information, see [about_CommonParameters](https://go.microsoft.com/fwlink/?LinkID=113216).</span></span>

## <span data-ttu-id="92b67-568">ENTRÉES</span><span class="sxs-lookup"><span data-stu-id="92b67-568">INPUTS</span></span>

### <span data-ttu-id="92b67-569">System. Management. Automation. ScriptBlock</span><span class="sxs-lookup"><span data-stu-id="92b67-569">System.Management.Automation.ScriptBlock</span></span>

<span data-ttu-id="92b67-570">Vous pouvez diriger une commande dans un bloc de script vers `Invoke-Command` .</span><span class="sxs-lookup"><span data-stu-id="92b67-570">You can pipe a command in a script block to `Invoke-Command`.</span></span> <span data-ttu-id="92b67-571">Utilisez la `$Input` variable Automatic pour représenter les objets d’entrée de la commande.</span><span class="sxs-lookup"><span data-stu-id="92b67-571">Use the `$Input` automatic variable to represent the input objects in the command.</span></span>

## <span data-ttu-id="92b67-572">SORTIES</span><span class="sxs-lookup"><span data-stu-id="92b67-572">OUTPUTS</span></span>

### <span data-ttu-id="92b67-573">System. Management. Automation. PSRemotingJob, System. Management. Automation. instances d’exécution. PSSession ou la sortie de la commande appelée</span><span class="sxs-lookup"><span data-stu-id="92b67-573">System.Management.Automation.PSRemotingJob, System.Management.Automation.Runspaces.PSSession, or the output of the invoked command</span></span>

<span data-ttu-id="92b67-574">Cette applet de commande retourne un objet de traitement, si vous utilisez le paramètre **AsJob** .</span><span class="sxs-lookup"><span data-stu-id="92b67-574">This cmdlet returns a job object, if you use the **AsJob** parameter.</span></span> <span data-ttu-id="92b67-575">Si vous spécifiez le paramètre **InDisconnectedSession** , `Invoke-Command` retourne un objet **PSSession** .</span><span class="sxs-lookup"><span data-stu-id="92b67-575">If you specify the **InDisconnectedSession** parameter, `Invoke-Command` returns a **PSSession** object.</span></span> <span data-ttu-id="92b67-576">Dans le cas contraire, elle retourne la sortie de la commande appelée, qui est la valeur du paramètre **scriptblock** .</span><span class="sxs-lookup"><span data-stu-id="92b67-576">Otherwise, it returns the output of the invoked command, which is the value of the **ScriptBlock** parameter.</span></span>

## <span data-ttu-id="92b67-577">REMARQUES</span><span class="sxs-lookup"><span data-stu-id="92b67-577">NOTES</span></span>

<span data-ttu-id="92b67-578">Sur Windows Vista et les versions ultérieures du système d’exploitation Windows, pour utiliser le paramètre **ComputerName** de `Invoke-Command` pour exécuter une commande sur l’ordinateur local, vous devez exécuter PowerShell à l’aide de l’option **exécuter en tant qu’administrateur** .</span><span class="sxs-lookup"><span data-stu-id="92b67-578">On Windows Vista, and later versions of the Windows operating system, to use the **ComputerName** parameter of `Invoke-Command` to run a command on the local computer, you must run PowerShell using the **Run as administrator** option.</span></span>

<span data-ttu-id="92b67-579">Lorsque vous exécutez des commandes sur plusieurs ordinateurs, PowerShell se connecte aux ordinateurs dans l’ordre dans lequel ils apparaissent dans la liste.</span><span class="sxs-lookup"><span data-stu-id="92b67-579">When you run commands on multiple computers, PowerShell connects to the computers in the order in which they appear in the list.</span></span> <span data-ttu-id="92b67-580">Toutefois, la sortie de la commande s’affiche dans l’ordre dans lequel elle est reçue des ordinateurs distants, ce qui peut être différent.</span><span class="sxs-lookup"><span data-stu-id="92b67-580">However, the command output is displayed in the order that it's received from the remote computers, which might be different.</span></span>

<span data-ttu-id="92b67-581">Les erreurs qui résultent de la commande qui `Invoke-Command` s’exécute sont incluses dans les résultats de la commande.</span><span class="sxs-lookup"><span data-stu-id="92b67-581">Errors that result from the command that `Invoke-Command` runs are included in the command results.</span></span>
<span data-ttu-id="92b67-582">Les erreurs qui seraient des erreurs avec fin d'exécution dans une commande locale sont traitées comme des erreurs sans fin d'exécution dans une commande à distance.</span><span class="sxs-lookup"><span data-stu-id="92b67-582">Errors that would be terminating errors in a local command are treated as non-terminating errors in a remote command.</span></span> <span data-ttu-id="92b67-583">Cette stratégie permet de s’assurer que les erreurs d’arrêt sur un ordinateur ne ferment pas la commande sur tous les ordinateurs sur lesquels elle est exécutée.</span><span class="sxs-lookup"><span data-stu-id="92b67-583">This strategy makes sure that terminating errors on one computer don't close the command on all computers on which it's run.</span></span> <span data-ttu-id="92b67-584">Cette pratique est utilisée même quand une commande à distance est exécutée sur un seul ordinateur.</span><span class="sxs-lookup"><span data-stu-id="92b67-584">This practice is used even when a remote command is run on a single computer.</span></span>

<span data-ttu-id="92b67-585">Si l’ordinateur distant n’est pas dans un domaine approuvé par l’ordinateur local, il se peut que l’ordinateur ne soit pas en mesure d’authentifier les informations d’identification de l’utilisateur.</span><span class="sxs-lookup"><span data-stu-id="92b67-585">If the remote computer isn't in a domain that the local computer trusts, the computer might not be able to authenticate the user's credentials.</span></span> <span data-ttu-id="92b67-586">Pour ajouter l’ordinateur distant à la liste des hôtes approuvés dans WS-Management, utilisez la commande suivante dans le `WSMAN` fournisseur, où `<Remote-Computer-Name>` est le nom de l’ordinateur distant :</span><span class="sxs-lookup"><span data-stu-id="92b67-586">To add the remote computer to the list of trusted hosts in WS-Management, use the following command in the `WSMAN` provider, where `<Remote-Computer-Name>` is the name of the remote computer:</span></span>

`Set-Item -Path WSMan:\Localhost\Client\TrustedHosts -Value \<Remote-Computer-Name\>`

<span data-ttu-id="92b67-587">Lorsque vous déconnectez une session **PSSession** à l’aide du paramètre **InDisconnectedSession** , l’état de session est **Disconnected** et la disponibilité est **None** .</span><span class="sxs-lookup"><span data-stu-id="92b67-587">When you disconnect a **PSSession** using the **InDisconnectedSession** parameter, the session state is **Disconnected** and the availability is **None** .</span></span> <span data-ttu-id="92b67-588">La valeur de la propriété **State** dépend de la session active.</span><span class="sxs-lookup"><span data-stu-id="92b67-588">The value of the **State** property is relative to the current session.</span></span> <span data-ttu-id="92b67-589">La valeur **Disconnected** signifie que la session **PSSession** n’est pas connectée à la session active.</span><span class="sxs-lookup"><span data-stu-id="92b67-589">A value of **Disconnected** means that the **PSSession** isn't connected to the current session.</span></span> <span data-ttu-id="92b67-590">Toutefois, cela ne signifie pas que la **session PSSession** est déconnectée de toutes les sessions.</span><span class="sxs-lookup"><span data-stu-id="92b67-590">However, it doesn't mean that the **PSSession** is disconnected from all sessions.</span></span> <span data-ttu-id="92b67-591">Elle peut être connectée à une autre session.</span><span class="sxs-lookup"><span data-stu-id="92b67-591">It might be connected to a different session.</span></span> <span data-ttu-id="92b67-592">Pour déterminer si vous pouvez vous connecter ou vous reconnecter à la session, utilisez la propriété **Availability** .</span><span class="sxs-lookup"><span data-stu-id="92b67-592">To determine whether you can connect or reconnect to the session, use the **Availability** property.</span></span>

<span data-ttu-id="92b67-593">Une propriété **Availability** avec la valeur **None** signifie que vous pouvez vous connecter à la session.</span><span class="sxs-lookup"><span data-stu-id="92b67-593">An **Availability** value of **None** indicates that you can connect to the session.</span></span> <span data-ttu-id="92b67-594">La valeur **Busy** indique que vous ne pouvez pas vous connecter à la session **PSSession** , car elle est connectée à une autre session.</span><span class="sxs-lookup"><span data-stu-id="92b67-594">A value of **Busy** indicates that you can't connect to the **PSSession** because it's connected to another session.</span></span> <span data-ttu-id="92b67-595">Pour plus d’informations sur les valeurs de la propriété **State** des sessions, consultez [RunspaceState](/dotnet/api/system.management.automation.runspaces.runspacestate).</span><span class="sxs-lookup"><span data-stu-id="92b67-595">For more information about the values of the **State** property of sessions, see [RunspaceState](/dotnet/api/system.management.automation.runspaces.runspacestate).</span></span> <span data-ttu-id="92b67-596">Pour plus d’informations sur les valeurs de la propriété **Availability** des sessions, consultez [RunspaceAvailability](/dotnet/api/system.management.automation.runspaces.runspaceavailability).</span><span class="sxs-lookup"><span data-stu-id="92b67-596">For more information about the values of the **Availability** property of sessions, see [RunspaceAvailability](/dotnet/api/system.management.automation.runspaces.runspaceavailability).</span></span>

<span data-ttu-id="92b67-597">Les paramètres **hostname** et **SSHConnection** ont été inclus à partir de PowerShell 6,0.</span><span class="sxs-lookup"><span data-stu-id="92b67-597">The **HostName** and **SSHConnection** parameters were included starting with PowerShell 6.0.</span></span> <span data-ttu-id="92b67-598">Elles ont été ajoutées pour fournir une communication à distance PowerShell basée sur Secure Shell (SSH).</span><span class="sxs-lookup"><span data-stu-id="92b67-598">They were added to provide PowerShell remoting based on Secure Shell (SSH).</span></span> <span data-ttu-id="92b67-599">PowerShell et SSH sont pris en charge sur plusieurs plateformes (Windows, Linux, macOS) et la communication à distance PowerShell fonctionne sur ces plateformes où PowerShell et SSH sont installés et configurés.</span><span class="sxs-lookup"><span data-stu-id="92b67-599">PowerShell and SSH are supported on multiple platforms (Windows, Linux, macOS) and PowerShell remoting works over these platforms where PowerShell and SSH are installed and configured.</span></span> <span data-ttu-id="92b67-600">Cela est différent de la version précédente de Windows uniquement qui est basée sur WinRM et la plupart des fonctionnalités et limitations spécifiques à WinRM ne s’appliquent pas.</span><span class="sxs-lookup"><span data-stu-id="92b67-600">This is separate from the previous Windows only remoting that is based on WinRM and many of the WinRM specific features and limitations don't apply.</span></span> <span data-ttu-id="92b67-601">Par exemple, les quotas basés sur WinRM, les options de session, la configuration de point de terminaison personnalisée et les fonctionnalités de déconnexion/reconnexion ne sont actuellement pas pris en charge.</span><span class="sxs-lookup"><span data-stu-id="92b67-601">For example WinRM based quotas, session options, custom endpoint configuration, and disconnect/reconnect features are currently not supported.</span></span> <span data-ttu-id="92b67-602">Pour plus d’informations sur la configuration de la communication à distance SSH de PowerShell, consultez [communication à distance PowerShell via SSH](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).</span><span class="sxs-lookup"><span data-stu-id="92b67-602">For more information about how to set up PowerShell SSH remoting, see [PowerShell Remoting Over SSH](/powershell/scripting/learn/remoting/ssh-remoting-in-powershell-core).</span></span>

## <span data-ttu-id="92b67-603">LIENS CONNEXES</span><span class="sxs-lookup"><span data-stu-id="92b67-603">RELATED LINKS</span></span>

[<span data-ttu-id="92b67-604">about_PSSessions</span><span class="sxs-lookup"><span data-stu-id="92b67-604">about_PSSessions</span></span>](./About/about_PSSessions.md)

[<span data-ttu-id="92b67-605">about_Remote</span><span class="sxs-lookup"><span data-stu-id="92b67-605">about_Remote</span></span>](./About/about_Remote.md)

[<span data-ttu-id="92b67-606">about_Remote_Disconnected_Sessions</span><span class="sxs-lookup"><span data-stu-id="92b67-606">about_Remote_Disconnected_Sessions</span></span>](./About/about_Remote_Disconnected_Sessions.md)

[<span data-ttu-id="92b67-607">about_Remote_Troubleshooting</span><span class="sxs-lookup"><span data-stu-id="92b67-607">about_Remote_Troubleshooting</span></span>](./About/about_remote_troubleshooting.md)

[<span data-ttu-id="92b67-608">about_Remote_Variables</span><span class="sxs-lookup"><span data-stu-id="92b67-608">about_Remote_Variables</span></span>](./About/about_Remote_Variables.md)

[<span data-ttu-id="92b67-609">about_Scopes</span><span class="sxs-lookup"><span data-stu-id="92b67-609">about_Scopes</span></span>](./About/about_scopes.md)

[<span data-ttu-id="92b67-610">Enter-PSSession</span><span class="sxs-lookup"><span data-stu-id="92b67-610">Enter-PSSession</span></span>](Enter-PSSession.md)

[<span data-ttu-id="92b67-611">Exit-PSSession</span><span class="sxs-lookup"><span data-stu-id="92b67-611">Exit-PSSession</span></span>](Exit-PSSession.md)

[<span data-ttu-id="92b67-612">Get-PSSession</span><span class="sxs-lookup"><span data-stu-id="92b67-612">Get-PSSession</span></span>](Get-PSSession.md)

[<span data-ttu-id="92b67-613">Invoke-Item</span><span class="sxs-lookup"><span data-stu-id="92b67-613">Invoke-Item</span></span>](../Microsoft.PowerShell.Management/Invoke-Item.md)

[<span data-ttu-id="92b67-614">New-PSSession</span><span class="sxs-lookup"><span data-stu-id="92b67-614">New-PSSession</span></span>](New-PSSession.md)

[<span data-ttu-id="92b67-615">Remove-PSSession</span><span class="sxs-lookup"><span data-stu-id="92b67-615">Remove-PSSession</span></span>](Remove-PSSession.md)

[<span data-ttu-id="92b67-616">Fournisseur WSMan</span><span class="sxs-lookup"><span data-stu-id="92b67-616">WSMan Provider</span></span>](../Microsoft.WsMan.Management/About/about_WSMan_Provider.md)
