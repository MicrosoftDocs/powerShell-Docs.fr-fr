...
À compter de PowerShell 7, `Group-Object` peut combiner les paramètres **CaseSensitive** et **AsHashtable** pour créer une table de hachage qui respecte la casse. Les clés de table de hachage utilisent des comparaisons sensibles à la casse et génèrent un objet **System. Collections. Hashtable** .
...
### Exemple 7 : créer une table de hachage respectant la casse

Cet exemple combine les paramètres **CaseSensitive** et **AsHashTable** pour créer une table de hachage qui respecte la casse. Les fichiers de l’exemple ont des extensions de `.txt` et `.TXT` .
...
La `$hash` variable stocke l’objet **System. Collections. Hashtable** . `Get-ChildItem` Obtient les noms de fichiers à partir du `C:\Files` répertoire et envoie les objets **System. IO. FileInfo** vers le dessous du pipeline. `Group-Object`groupe les objets à l’aide de l' **extension** de valeur de **propriété** . Les paramètres **CaseSensitive** et **AsHashTable** créent la table de hachage et les clés sont regroupées à l’aide des clés sensibles à la casse `.txt` et `.TXT` .
