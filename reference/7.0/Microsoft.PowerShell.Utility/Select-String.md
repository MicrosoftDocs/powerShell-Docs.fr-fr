...
- CurrentCulture, qui est la valeur par défaut ;
- Ordinal, autrement dit une comparaison binaire non linguistique ;
- Invariant, qui est une comparaison indépendante de la culture.

Avec la `Select-String -Culture Ordinal -CaseSensitive -SimpleMatch` commande, vous allez obtenir une comparaison binaire plus rapide.

Le paramètre **culture** utilise la saisie semi-automatique via la touche Tab pour faire défiler la liste des arguments qui spécifient les cultures disponibles. Pour répertorier tous les arguments disponibles, utilisez la commande suivante :

`(Get-Command Select-String).Parameters.Culture.Attributes.ValidValues`

Pour plus d’informations sur la propriété .NET CultureInfo.Name, consultez [CultureInfo.Name](/dotnet/api//system.globalization.cultureinfo.name).
...
Par défaut, `Select-String` met en surbrillance la chaîne qui correspond au modèle que vous avez recherché avec le paramètre **pattern** . Le paramètre **norelief** désactive la mise en surbrillance.

L’accentuation utilise des couleurs négatives en fonction de vos couleurs d’arrière-plan et de texte PowerShell. Par exemple, si vos couleurs PowerShell sont un arrière-plan noir avec du texte blanc. L’accent est un arrière-plan blanc avec du texte noir.
...
Fait en sorte que l’applet de commande génère uniquement les chaînes correspondantes, plutôt que les objets **MatchInfo** . Il s’agit du comportement qui est le plus semblable aux commandes d’UNIX **grep** ou Windows **findstr.exe** .
...
Si vous utilisez le paramètre **RAW** , la sortie est un ensemble d’objets **String** qui correspondent au modèle.
