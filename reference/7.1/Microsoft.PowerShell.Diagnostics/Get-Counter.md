...
> [!NOTE]
> Dans PowerShell 7, `Get-Counter` Impossible de récupérer la propriété **Description** de l’ensemble de compteurs. La **Description** a la valeur `$null` .
...
Dans PowerShell 7, quand vous utilisez le paramètre **ListSet** , vous `Get-Counter` ne pouvez pas récupérer la propriété **Description** de l’ensemble de compteurs. La **Description** a la valeur `$null` .
