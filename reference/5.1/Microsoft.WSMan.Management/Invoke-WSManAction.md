...
Invoke-WSManAction exécute une action sur l'objet spécifié par URI_RESSOURCE, où les paramètres sont spécifiés par les paires clé/valeur.
...
Cette commande appelle la méthode StartService de l'instance de classe WMI Win32_Service qui correspond au service Spooler.
Comme le paramètre ComputerName est spécifié, la commande s'exécute sur l'ordinateur distant server01.
...
Ce paramètre est conçu pour être utilisé quand de nombreux ordinateurs établissent des connexions à distance à un ordinateur qui exécute Windows PowerShell.
...
Passe un ensemble de commutateurs à un service pour modifier ou affiner la nature de la demande.
