...
> [!IMPORTANT]
> `Update-Help` requiert des privilèges d’administrateur.
>
...
>
> Vous pouvez également mettre à jour les fichiers d’aide à l’aide de l’élément de menu mettre à jour l’aide de Windows PowerShell dans le menu aide de Environnement d’écriture de scripts intégré de Windows PowerShell (ISE).
>
> L’élément d’aide mettre à jour Windows PowerShell exécute une applet de commande `Update-Help` sans paramètres.
> Pour mettre à jour l’aide pour les modules dans l' `$PSHOME` annuaire, démarrez Windows PowerShell ISE à l’aide de l’option Exécuter en tant qu’administrateur.
...
### Exemple 4 : mettre à jour les fichiers d’aide automatiquement

Cet exemple crée une tâche planifiée qui met à jour l’aide de tous les modules tous les jours à 3:00 AM.
...
L' `Register-ScheduledJob` applet de commande crée une tâche planifiée qui exécute une `Update-Help` commande. La commande utilise le paramètre **Credential** pour s’exécuter à l' `Update-Help` aide des informations d’identification d’un membre du groupe Administrateurs sur l’ordinateur. La valeur du paramètre **Trigger** est une `New-JobTrigger` commande qui crée un déclencheur de tâche qui démarre le travail tous les jours à 3:00 AM.

Pour exécuter la `Register-ScheduledJob` commande, démarrez PowerShell à l’aide de l’option **exécuter en tant qu’administrateur** . PowerShell vous invite à entrer le mot de passe de l’utilisateur spécifié dans le paramètre **Credential** . Les informations d’identification sont stockées avec la tâche planifiée. Vous n’êtes pas invité à l’exécution de la tâche.

Vous pouvez utiliser l' `Get-ScheduledJob` applet de commande pour afficher la tâche planifiée, utiliser l' `Set-ScheduledJob` applet de commande pour la modifier et utiliser l' `Unregister-ScheduledJob` applet de commande pour la supprimer. Vous pouvez également afficher et gérer la tâche planifiée dans le Planificateur de tâches dans le chemin d'accès suivant :
...
### Exemple 7 : Rechercher les modules qui prennent en charge l’aide actualisable
...
### Exemple 8 : inventaire des fichiers d’aide mis à jour
