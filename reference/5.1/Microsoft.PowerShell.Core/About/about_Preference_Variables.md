...
| `$OFS`                           | (Espace ( `" "` )) |
| `$OutputEncoding`                | **ASCIIEncoding** , objet  |
...
- **Suspend** : suspend automatiquement une tâche de workflow après `Write-Information` l’exécution d’une commande, pour permettre aux utilisateurs de voir les messages avant de continuer. Le flux de travail peut être repris à la discrétion de l’utilisateur.
...
Détermine le nombre d’alias autorisés dans une session PowerShell. La valeur par défaut est **4096** et cela doit être suffisant pour la plupart des utilisations. Vous pouvez ajuster `$MaximumAliasCount` pour répondre à vos besoins.

**Valeurs valides** : 1024-32768 ( `Int32` )

**Valeur par défaut** : 4096

Pour compter les alias sur votre système, tapez :
...
Détermine le nombre de lecteurs PowerShell autorisés dans une session donnée. Par exemple, les lecteurs du système de fichiers et les magasins de données qui sont exposés par les fournisseurs PowerShell et apparaissent en tant que lecteurs, tels que les `Alias:` `HKLM:` lecteurs et.
...
Les objets qui représentent chaque erreur conservée sont stockés dans la `$Error` variable automatique. `$Error` contient un tableau d’objets d’enregistrement d’erreur. L’erreur la plus récente est le premier objet du tableau, `$Error[0]` .

Pour compter les erreurs sur votre système, utilisez la `$Error` propriété **Count** du tableau.

```powershell
$Error.count
```

Pour afficher une erreur spécifique, utilisez la `[0]` notation de tableau pour voir l’erreur la plus récente.
...
Le paramètre **force** remplace la mise en forme spéciale des objets **ErrorRecord** et revient au format conventionnel. Pour afficher les propriétés de l’objet **ErrorRecord** , tapez la commande suivante :

```powershell
$Error[0] | Format-List -Property * -Force
```

Dans cet exemple, `$Error.Count` affiche le nombre d’erreurs. Pour supprimer toutes les erreurs de l’historique des erreurs, utilisez la `Clear` méthode du tableau d’erreurs.
...
Pour rechercher toutes les propriétés et méthodes d’un tableau d’erreurs, utilisez l' `Get-Member` applet de commande avec son paramètre **InputObject** . Quand vous utilisez le paramètre **InputObject** , `Get-Member` affiche les propriétés et les méthodes de la collection.

```powershell
Get-Member -InputObject $Error
```

Quand vous dirigez une collection d’objets vers `Get-Member` , `Get-Member` affiche les propriétés et les méthodes des objets dans la collection.
...
Pour afficher les fonctions dans votre session, utilisez le `Function:` lecteur PowerShell qui est exposé par le `Function` fournisseur PowerShell. Pour plus d’informations sur le `Function` fournisseur, [about_Function_Provider](about_Function_Provider.md).
...
Détermine le nombre de variables autorisées dans une session donnée, y compris les variables automatiques, les variables de préférence et les variables que vous créez dans les commandes et les scripts.

**Valeurs valides** : 1024-32768 ( `Int32` )

**Valeur par défaut** : 4096

Pour afficher les variables dans votre session, utilisez l' `Get-Variable` applet de commande et les fonctionnalités du `Variable:` lecteur PowerShell et du `Variable` fournisseur PowerShell. Pour plus d’informations, consultez [about_Variable_Provider](about_Variable_Provider.md).
