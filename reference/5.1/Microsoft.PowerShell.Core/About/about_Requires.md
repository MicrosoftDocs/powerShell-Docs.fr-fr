...
Exiger que `Hyper-V` (version `1.1` ou ultérieure) soit installé.
...
Requiert l' `Hyper-V` installation de (version `1.1` ou inférieure).

```powershell
#Requires -Modules @{ ModuleName="Hyper-V"; MaximumVersion="1.1" }
```

Requiert l’installation de n’importe quelle version de `PSScheduledJob` et de `PSWorkflow` .

```powershell
#Requires -Modules PSWorkflow, PSScheduledJob
```

Lorsque vous utilisez la `RequiredVersion` clé, vérifiez que votre chaîne de version correspond exactement à la chaîne de version que vous souhaitez exiger.
