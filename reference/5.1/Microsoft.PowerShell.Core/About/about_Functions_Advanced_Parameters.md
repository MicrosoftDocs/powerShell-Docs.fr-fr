...
Il existe un problème connu lié à l’utilisation de collections avec **ValueFromRemainingArguments** où la collection passée est traitée comme un élément unique.

L’exemple suivant illustre ce problème connu. Le paramètre **restant** doit contenir **un à l'** **index 0** et **deux** à l' **index 1**.
Au lieu de cela, les deux éléments sont combinés dans une entité unique.
...
> [!NOTE]
> Ce problème est résolu dans PowerShell 6,2.
...
L’attribut **ValidateRange** spécifie une plage numérique pour chaque valeur de paramètre ou de variable. PowerShell génère une erreur si une valeur est en dehors de cette plage. Dans l’exemple suivant, la valeur du paramètre **tentatives** doit être comprise entre zéro et dix.
