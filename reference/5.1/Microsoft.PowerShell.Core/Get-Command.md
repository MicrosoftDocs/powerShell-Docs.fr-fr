...
Pour obtenir la propriété **PSTypeNames** d'une carte réseau, et non la propriété **PSTypeNames** d'une collection de cartes réseau, la commande utilise une notation de tableau. Ainsi, elle obtient la première carte réseau retournée par l'applet de commande.
...
- Flux. obtient tous les workflows. Pour plus d'informations sur les workflows, consultez Introducing Windows PowerShell Workflow.
...
Ce paramètre accepte les valeurs de chaîne, mais la valeur de ce paramètre peut également être un objet **PSModuleInfo** ou **PSSnapinInfo** , tel que les objets que les `Get-Module` applets de commande, `Get-PSSnapin` et `Import-PSSession` retournent.

Vous pouvez faire référence à ce paramètre par son nom, **Module** , ou par son alias, **PSSnapin** .
Le nom de paramètre que vous choisissez n'a aucun effet sur la sortie de commande.
