...
### Exemple 1 : modifier l’état de cloisonnement du thread

```
PS C:\> Set-PSSessionConfiguration -Name "MaintenanceShell" -ThreadApartmentState STA
```

Cette commande définit sur STA l'état de cloisonnement des threads dans la configuration MaintenanceShell.
La modification est effective quand vous redémarrez le service **WinRM** .
...
Spécifie le paramètre d’état de cloisonnement pour les threads dans la session.
Les valeurs acceptables pour ce paramètre sont les suivantes : STA, MTA et Unknown.
