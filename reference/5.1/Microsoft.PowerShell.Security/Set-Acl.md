...
Établit ou change la stratégie d'accès centralisée de l'élément.
Entrez l'ID ou le nom convivial d'une stratégie d'accès centralisée sur l'ordinateur.

À compter de Windows Server 2012, les administrateurs peuvent utiliser Active Directory et stratégie de groupe pour définir des stratégies d’accès centralisées pour les utilisateurs et les groupes.
Pour plus d’informations, consultez [Access Control dynamique : vue d’ensemble du scénario](/windows-server/identity/solution-guides/dynamic-access-control--scenario-overview).
