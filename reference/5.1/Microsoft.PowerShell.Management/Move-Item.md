...
L' `Move-Item` applet de commande déplace un élément, y compris ses propriétés, son contenu et ses éléments enfants, d’un emplacement à un autre.
...
Cette commande déplace le fichier « Test.txt » du `C:` lecteur vers le répertoire « E:\temp » et le renomme « test.txt » en « tst.txt ».
...
Remarque : dans Windows PowerShell 2.0, lorsque vous utilisez le paramètre **Recurse** de l'applet de commande `Get-ChildItem`, la valeur du paramètre **Path** doit être un conteneur.
...
Spécifie, sous la forme d’un tableau de chaînes, un ou des éléments que cette applet de commande déplace dans l’opération.
La valeur de ce paramètre qualifie le paramètre **Path** .
Entrez un élément ou un modèle de chemin d'accès, tel que « *.txt ».
Les caractères génériques sont autorisés.
