...
Vous pouvez également utiliser le paramètre **StackName** pour faire d’une pile d’emplacements nommée la pile d’emplacements active. Pour plus d'informations sur les piles d'emplacements, consultez les remarques.
...
Les `*-Location` applets de commande agissent sur la pile active, sauf si vous utilisez le paramètre **StackName** pour spécifier une autre pile.
