...
Lorsqu’une chaîne d’espace blanc est fournie pour le `-Path` paramètre, elle retourne **true** . Lorsqu’une chaîne vide est fournie, `Test-Path` retourne une erreur. L’exemple suivant montre un espace blanc et une chaîne vide.
