...
Indique que cette applet de commande charge le profil utilisateur Windows stocké dans la `HKEY_USERS` clé de registre de l’utilisateur actuel.
...
Démarrez le nouveau processus dans la fenêtre de console active. Par défaut, PowerShell ouvre une nouvelle fenêtre.
