...
Quand vous vous abonnez à un événement à l'aide d'une applet de commande d'événement Register, un abonné aux événements est ajouté à votre session Windows PowerShell et les événements auxquels vous vous êtes abonné sont ajoutés à la file d'attente d'événements chaque fois qu'ils sont déclenchés.
