...
L' `Export-FormatData` applet de commande crée des fichiers de mise en forme Windows PowerShell (format.ps1XML) à partir des objets de mise en forme de la session active. Elle prend les objets **ExtendedTypeDefinition** qui `Get-FormatData` sont retournées et les enregistre dans un fichier au format XML.
...
Pour plus d’informations sur la mise en forme des fichiers dans Windows PowerShell, consultez [about_Format.ps1XML](../Microsoft.PowerShell.Core/About/about_Format.ps1xml.md).
