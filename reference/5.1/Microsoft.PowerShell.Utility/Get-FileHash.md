...
Cet exemple utilise l' `Get-FileHash` applet de commande pour calculer la valeur de hachage du `Powershell.exe` fichier.
L'algorithme de hachage utilisé est la valeur par défaut, SHA256. La sortie est dirigée vers l' `Format-List` applet de commande pour mettre en forme la sortie sous forme de liste.
