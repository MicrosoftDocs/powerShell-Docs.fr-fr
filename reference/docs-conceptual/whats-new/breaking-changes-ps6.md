...
En raison de l’utilisation des API non prises en charge, `*-EventLog` a été supprimé de PowerShell Core jusqu’à ce qu’une meilleure solution soit trouvée. `Get-WinEvent` et `New-WinEvent` sont disponibles pour obtenir et créer des événements sur Windows.
