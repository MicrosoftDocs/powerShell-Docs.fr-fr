...
L’exemple suivant montre un élément `WideEntry` qui définit un élément `WideItem` unique. L’élément `WideItem` définit la propriété dont la valeur est affichée dans la vue.
