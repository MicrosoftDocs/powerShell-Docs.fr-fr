...
L’exemple suivant montre un élément `WideEntries` qui définit un seul élément `WideEntry`. L’élément `WideEntry` contient un seul élément `WideItem` qui définit la valeur de la propriété ou du script qui est affichée dans la vue.
