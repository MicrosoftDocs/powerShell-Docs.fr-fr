...
Lorsque vous nommez la classe .NET Framework qui implémente une applet de commande, nommez la classe « *\<Verb > **\<Noun >** \<Command >* », où vous remplacez les espaces réservés @no__t *-6Verb* > et *\<Noun >* par le verbe et le substantif utilisés pour le nom de l’applet de commande. Par exemple, l’applet de commande « [obtenir-process »](/powershell/module/Microsoft.PowerShell.Management/Get-Process) est implémentée par une classe appelée `GetProcessCommand`.
