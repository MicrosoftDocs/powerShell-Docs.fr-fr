...
Une liste complète des verbes peut être affichée à l’aide de l’applet de commande `Get-Verb` .
...
|[Nouveau](/dotnet/api/System.Management.Automation.VerbsCommon.New) (n)|Crée une ressource. (Le `Set` verbe peut également être utilisé lors de la création d’une ressource qui comprend des données, telles que l’applet de commande `Set-Variable` .)|Pour cette action, n’utilisez pas de verbes tels que Create, Generate, Build, make ou Allocate.|
...
|[Redimensionner](/dotnet/api/System.Management.Automation.VerbsCommon.Resize)(RZ)|Modifie la taille d’une ressource.||
