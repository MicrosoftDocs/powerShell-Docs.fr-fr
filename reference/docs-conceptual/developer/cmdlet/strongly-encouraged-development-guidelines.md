...
Définir des membres standard pour étendre un type d’objet dans un fichier XML Types.ps1personnalisé (utilisez le fichier XML Windows PowerShell Types.ps1en tant que modèle). Les membres standard sont définis par un nœud portant le nom PSStandardMembers. Ces définitions autorisent d’autres applets de commande et le runtime Windows PowerShell à travailler avec votre objet de manière cohérente.
...
Si l’affichage d’un objet ne fournit pas les résultats attendus, créez un *\<YourProductName>* fichier XML de.Format.ps1personnalisé pour cet objet.
