---
title: Exemples de code GetProc02 | Microsoft Docs
ms.date: 09/13/2016
ms.openlocfilehash: 463786cfa679b89f29a3acb6b40a31409634d256
ms.sourcegitcommit: 0907b8c6322d2c7c61b17f8168d53452c8964b41
ms.translationtype: MT
ms.contentlocale: fr-FR
ms.lasthandoff: 08/05/2020
ms.locfileid: "87778738"
---
# <a name="getproc02-code-samples"></a>Exemples de code GetProc02

Voici les exemples de code pour l’applet de commande GetProc02 Sample. Il s’agit de l' `Get-Process` exemple d’applet de commande décrit dans [Ajout de paramètres qui traitent l’entrée de ligne de commande](../cmdlet/adding-parameters-that-process-command-line-input.md). Cette `Get-Process` applet de commande récupère les processus en fonction de leur nom, puis affiche des informations sur les processus sur la ligne de commande.

> [!NOTE]
> Vous pouvez télécharger le fichier source C# (getproc02.cs) pour cette applet de commande « obtenir-proc » à l’aide du kit de développement logiciel (SDK) Microsoft Windows pour les composants d’exécution de Windows Vista et .NET Framework 3,0. Pour obtenir des instructions de téléchargement, consultez [Comment installer Windows PowerShell et télécharger le kit de développement logiciel (SDK) Windows PowerShell](/powershell/scripting/developer/installing-the-windows-powershell-sdk).
>
> Les fichiers sources téléchargés sont disponibles dans le **\<PowerShell Samples>** répertoire.

Pour obtenir un exemple de code complet, consultez les rubriques suivantes.

|Langage|Rubrique|
|--------------|-----------|
|C#|[Exemple de code GetProc02 (C#)](./getproc02-csharp-sample-code.md)|
|VB.NET|[Exemple de code GetProc02 (VB.NET)](./getproc02-vb-net-sample-code.md)|

## <a name="see-also"></a>Voir aussi

[Guide de programmation pour Windows PowerShell](./windows-powershell-programmer-s-guide.md)

[Windows PowerShell SDK](../windows-powershell-reference.md)
