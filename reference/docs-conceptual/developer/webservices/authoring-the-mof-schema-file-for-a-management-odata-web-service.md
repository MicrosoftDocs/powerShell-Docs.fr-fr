...
Vous définissez les ressources exposées par votre service Web OData de gestion en créant un fichier MOF qui utilisait le schéma de ressource publique. Chaque ressource est définie en tant que classe dans le fichier, et les propriétés sont définies comme membres de classe. Pour plus d’informations sur le schéma utilisé dans le fichier MOF, consultez [schéma de ressources publiques](./public-resource-schema.md).
