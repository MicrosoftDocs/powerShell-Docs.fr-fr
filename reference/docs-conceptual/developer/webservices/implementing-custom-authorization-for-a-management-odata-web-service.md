---
title: Implémentation d’une autorisation personnalisée pour un service Web OData de gestion | Microsoft Docs
ms.custom: ''
ms.date: 09/13/2016
ms.reviewer: ''
ms.suite: ''
...
---
# <a name="implementing-custom-authorization-for-a-management-odata-web-service"></a><span data-ttu-id="6c4c0-102">Implémentation d’une autorisation personnalisée pour un service web Management OData</span><span class="sxs-lookup"><span data-stu-id="6c4c0-102">Implementing Custom Authorization for a Management OData web service</span></span>

<span data-ttu-id="6c4c0-103">L’utilisation du service Web Windows PowerShell nécessite une partie tierce pour implémenter l’interface [Microsoft. Management. OData. CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) pour exposer les applets de commande Windows PowerShell.</span><span class="sxs-lookup"><span data-stu-id="6c4c0-103">Using the Windows PowerShell Web Service requires a third party to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface to expose Windows PowerShell cmdlets.</span></span> <span data-ttu-id="6c4c0-104">Cette interface effectue l’autorisation de l’utilisateur sur le service Web.</span><span class="sxs-lookup"><span data-stu-id="6c4c0-104">This interface performs user authorization to the web service.</span></span> <span data-ttu-id="6c4c0-105">Après avoir écrit le code pour implémenter l’interface, vous devez le compiler dans une DLL à utiliser dans l’application Web.</span><span class="sxs-lookup"><span data-stu-id="6c4c0-105">After writing the code to implement the interface, you must compile it into a DLL to be used in the web application.</span></span>

## <a name="pass-through-authorization"></a><span data-ttu-id="6c4c0-106">Autorisation directe</span><span class="sxs-lookup"><span data-stu-id="6c4c0-106">Pass-through authorization</span></span>

<span data-ttu-id="6c4c0-107">La façon la plus simple d’implémenter l’interface [Microsoft. Management. OData. CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) est une implémentation directe qui autorise tous les utilisateurs.</span><span class="sxs-lookup"><span data-stu-id="6c4c0-107">The simplest way to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface is a pass-through implementation that authorizes all users.</span></span> <span data-ttu-id="6c4c0-108">Cet exemple ne fournit aucune sécurité et n’est fourni que comme une illustration de la manière d’implémenter l’interface.</span><span class="sxs-lookup"><span data-stu-id="6c4c0-108">This example provides no security, and s provided only as an illustration of how to implement the interface.</span></span> <span data-ttu-id="6c4c0-109">Une implémentation de l’interface [Microsoft. Management. OData. CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) doit substituer deux méthodes : [Microsoft. Management. OData. CustomAuthorization. AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) et [Microsoft. Management. OData. CustomAuthorization. GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span><span class="sxs-lookup"><span data-stu-id="6c4c0-109">An implementation of the  [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface must override two methods: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span></span> <span data-ttu-id="6c4c0-110">Dans cet exemple, [Microsoft. Management. OData. CustomAuthorization. AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) retourne toujours l’objet **System. Security. principal. WindowsIdentity** associé à l’utilisateur actuel.</span><span class="sxs-lookup"><span data-stu-id="6c4c0-110">In this example, the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) always returns the **System.Security.Principal.WindowsIdentity** object associated with the current user.</span></span>
...
### <a name="role-based-authorization"></a><span data-ttu-id="6c4c0-111">Autorisation basée sur des rôles</span><span class="sxs-lookup"><span data-stu-id="6c4c0-111">Role-based authorization</span></span>

<span data-ttu-id="6c4c0-112">L’exemple suivant implémente une stratégie d’autorisation basée sur les rôles.</span><span class="sxs-lookup"><span data-stu-id="6c4c0-112">The following example implements a role-based authorization policy.</span></span> <span data-ttu-id="6c4c0-113">La stratégie est définie dans un fichier XML qui réside dans le répertoire principal de l’application avec les fichiers de schéma de mappage Web. config et MOF et XML.</span><span class="sxs-lookup"><span data-stu-id="6c4c0-113">The policy is defined in an XML file that resides in the main application directory with the web.config and MOF and XML mapping schema files.</span></span> <span data-ttu-id="6c4c0-114">Pour plus d’informations sur la configuration du fichier de schéma d’autorisation, consultez Configuration de l' [autorisation basée sur les rôles](./configuring-role-based-authorization.md).</span><span class="sxs-lookup"><span data-stu-id="6c4c0-114">For information about how to configure the authorization schema file, see [Configuring Role-based Authorization](./configuring-role-based-authorization.md).</span></span> <span data-ttu-id="6c4c0-115">La première partie de l’exemple implémente les méthodes [Microsoft. Management. OData. CustomAuthorization. AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) et [Microsoft. Management. OData. CustomAuthorization. GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) .</span><span class="sxs-lookup"><span data-stu-id="6c4c0-115">The first part of the sample implements the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) methods.</span></span> <span data-ttu-id="6c4c0-116">Dans ce cas, les méthodes d’interface appellent les méthodes de la classe `RbacSystem` (définie ci-dessous) qui effectuent le travail réel de vérification des autorisations pour l’utilisateur.</span><span class="sxs-lookup"><span data-stu-id="6c4c0-116">In this case, the interface methods call methods in the `RbacSystem` class (defined below) that do the actual work of checking the permissions for the user.</span></span>
...
<span data-ttu-id="6c4c0-117">L’exemple suivant crée une classe qui analyse la stratégie d’autorisation dans le fichier XML.</span><span class="sxs-lookup"><span data-stu-id="6c4c0-117">The following example creates a class that parses authorization policy in the XML file.</span></span>
...
```
 ...
 <span data-ttu-id="6c4c0-118">Les classes suivantes représentent des groupes et des utilisateurs, respectivement.</span><span class="sxs-lookup"><span data-stu-id="6c4c0-118">The following classes represent groups and users, respectively.</span></span>
...
<span data-ttu-id="6c4c0-119">Enfin, la classe RbacSystem implémente des méthodes qui effectuent le travail de vérification des autorisations pour l’utilisateur et retournent l’état d’autorisation aux méthodes définies dans l’implémentation de l’interface [Microsoft. Management. OData. CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) .</span><span class="sxs-lookup"><span data-stu-id="6c4c0-119">Finally, the RbacSystem class implements methods that do the work of checking the permissions for the user and return the authorization status to the methods defined in the implementation of the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface.</span></span>