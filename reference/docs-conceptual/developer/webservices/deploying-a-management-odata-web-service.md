...
Après avoir effectué toutes les étapes nécessaires à la création d’un service Web OData de gestion, vous devez le déployer en tant qu’application Web dans IIS.

## <a name="deploying-the-web-service"></a>Déploiement du service Web

Procédez comme suit pour déployer le service Web OData de gestion.

1. Créez un répertoire pour votre application Web dans votre répertoire de `WWWRoot` IIS.
...
3. Créez un ID de site.

4. Créez et configurez un pool d’applications.

5. Configurez l’authentification pour le site.

6. Configurez le pare-feu.

7. Démarrez le site.

Les scripts Windows PowerShell suivants montrent comment déployer un service Web OData de gestion.
