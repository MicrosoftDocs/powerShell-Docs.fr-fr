...
> [!CAUTION]
> Si vous créez un **SymbolicLink** dans un dossier sous Windows, vous devez utiliser un chemin d’accès complet. Si vous utilisez un chemin d’accès relatif, le lien est créé en tant que lien de type de fichier au lieu d’un lien de type répertoire.
