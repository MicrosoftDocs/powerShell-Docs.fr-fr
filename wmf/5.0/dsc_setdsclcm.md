...
ms.locfileid: "62057944"
---
# <a name="set-dsclocalconfigurationmanager-cmdlet-supports--force-parameter"></a><span data-ttu-id="65ae7-102">L’applet de commande Set-DscLocalConfigurationManager prend en charge le paramètre -force</span><span class="sxs-lookup"><span data-stu-id="65ae7-102">Set-DscLocalConfigurationManager cmdlet supports -force parameter</span></span>

<span data-ttu-id="65ae7-103">Nous avons ajouté la prise en charge d’un nouveau paramètre à l’applet de commande Set-DscLocalConfigurationManager.</span><span class="sxs-lookup"><span data-stu-id="65ae7-103">We have added a support for new parameter to Set-DscLocalConfigurationManager cmdlet.</span></span> <span data-ttu-id="65ae7-104">Il permet à l’utilisateur de réinitialiser la métaconfiguration sur l’ordinateur de façon déterministe quand d’autres opérations (telles que la vérification de cohérence) s’exécutent en arrière-plan, car cela entraîne l’arrêt de toutes les opérations en cours d’exécution.</span><span class="sxs-lookup"><span data-stu-id="65ae7-104">This will allow the user to reset meta configuration on machine deterministically when other operations like consistency check are running in background as it will cause all running operations to be stopped.</span></span>

<span data-ttu-id="65ae7-105">L’expérience ressemble à ceci quand vous tentez de définir une métaconfiguration sans le paramètre -force.</span><span class="sxs-lookup"><span data-stu-id="65ae7-105">The experience looks like this when trying to set meta configuration without –Force parameter.</span></span>
```powershell
